...
public class BezierKnot extends Object implements Cloneable {
    ...
    public int compareTo(Object other) {
        ...
        if (equals(otherControlPoint)) return 0;
    ...
    public boolean equals(Object other) {
        BezierKnot otherControlPoint = (BezierKnot) other;
        for (int i = 0; i < 3; i++) {
            if (getPoints()[i].x != otherControlPoint.getPoints()[i].x) return false;

            if (getPoints()[i].y != otherControlPoint.getPoints()[i].y) return false;
        }

        if (mContinous != otherControlPoint.mContinous) return false;

        if (getOther() != otherControlPoint.getOther()) return false;
