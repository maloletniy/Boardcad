...
public class BezierCurve implements Cloneable, BezierKnotChangeListener {
    ...
    public BezierCurve(BezierKnot startKnot, BezierKnot endKnot) {
        ...
        if (mStartKnot != null) mStartKnot.addChangeListener(this);
        if (mEndKnot != null) mEndKnot.addChangeListener(this);
    }
    ...
    private void calculateCoeff() {
        ...
        // y = ((Dt + E)t + F)t + G
        /*
         * double A = p3.x - (3*t2.x) + (3*t1.x) - p0.x; //A = x3 - 3x2 + 3x1 - x0 double B = (3*t2.x) - (6*t1.x) + (3*p0.x);
         * //B = 3x2 - 6x1 + 3x0 double C = (3*t1.x) - (3*p0.x); //C = 3x1 - 3x0 double D = p0.x; //D = x0
         * 
         * double E = p3.y - (3*t2.y) + (3*t1.y) - p0.y; //E = y3 - 3y2 + 3y1 - y0 double F = (3*t2.y) - (6*t1.y) + (3*p0.y);
         * //F = 3y2 - 6y1 + 3y0 double G = (3*t1.y) - (3*p0.y); //G = 3y1 - 3y0 double H = p0.y; //H = y0
         * 
         * coeff0 = A; coeff1 = B; coeff2 = C; coeff3 = D; coeff4 = E; coeff5 = F; coeff6 = G; coeff7 = H;
         */
        ...
        mCoeffDirty = false;
        /*
         * if(coeff0 != A) return; if(coeff1 != B) return; if(coeff2 != C) return; if(coeff3 != D) return; if(coeff4 != E)
         * return; if(coeff5 != F) return; if(coeff6 != G) return; if(coeff7 != H) return;
         */
        // Debug copyCoeff();
    }

    /*
     * Debug void copyCoeff() { for(int i = 0; i < 8; i++) { coeffCopy[i] = coeff[i]; } }
     * 
     * void compareCoeff() { for(int i = 0; i < 8; i++) { double val2 = coeffCopy[i]; double val1 = coeff[i];
     * 
     * double diff = Math.abs(val1 - val2); if(diff > 0.1) { System.out.println("Coeff changed i:" + i + " diff:" + diff +
     * " values:" + val1 + " " + val2); } } }
     */
    ...
    private double getTForXInternal(final double x, final double start_t) {
        ...
        while (Math.abs(error) > BezierSpline.POS_TOLERANCE && n++ < BezierSpline.POS_MAX_ITERATIONS) {
            ...
            error = x - xn;
            /*
             * if(Math.abs(error) > Math.abs(lasterror)) System.out.println("getTForX(): increasing error: " + error +
             * " last error:" + lasterror + " slope:" + currentSlope + " tn:" + tn);
             * 
             * lasterror = error;
             */ }

        // Sanity check
        if (tn < 0 || tn > 1 || Double.isNaN(tn) || n >= BezierSpline.POS_MAX_ITERATIONS
    ...
    private double getTForX(final double x, double t0, double t1, int nrOfSplits) {
        ...
        for (int i = 1; i < nrOfSplits; i++) {
            current_t = seg * i + t0;
            if (current_t < 0 || current_t > 1) continue;
    ...
    double getTForTangent2(double target_angle, double current_t, double last_t) {
        ...
        int n = 0;
        while (Math.abs(current_error) > BezierSpline.ANGLE_TOLERANCE && n++ < BezierSpline.ANGLE_MAX_ITERATIONS
            ...
            // System.out.printf("getTForTangent2(): current_slope:%f, current_error:%f, current_t:%f last_angle:%f
            // current_angle:%f target_angle:%f\n", current_slope, current_error/BezierBoard.DEG_TO_RAD, current_t,
            // last_angle/BezierBoard.DEG_TO_RAD, current_angle/BezierBoard.DEG_TO_RAD, target_angle/BezierBoard.DEG_TO_RAD);
        ...
        if (Math.abs(getTangent(current_t) - target_angle) > BezierSpline.ANGLE_TOLERANCE
            ...
            while (Math.abs(current_error) > BezierSpline.ANGLE_TOLERANCE && n++ < BezierSpline.ANGLE_MAX_ITERATIONS
                ...
                // System.out.printf("getTForTangent2(): current_error:%f, current_t:%f lt:%f ht:%f current_angle:%f
    ...
    synchronized double getYForX(final double x) {
         ...
         * 
         * if(Math.abs(xvalue - x) > POS_TOLERANCE) { System.out.println("find x: " + x + " real x: " + xvalue + " t:" + t +
    ...
    synchronized double getTForLength(
            Point2D.Double p0,
            Point2D.Double t1,
            Point2D.Double t2,
            Point2D.Double p3,
