...
public class BezierSpline implements Cloneable {
    ...
    public int indexOf(BezierKnot controlPoint) {
        for (int i = 0; i < mCurves.size() + 1; i++) {
            if (controlPoint == getControlPoint(i)) return i;
        }
        return -1;
    }
    ...
    public double getLengthByTangentReverse(double targetAngle, boolean useMinimumAngleOnSharpCorners) {
        ...
        for (i = mCurves.size() - 1; i >= 0; i--) {
            ...
            // startAngle/BezierBoard.DEG_TO_RAD +90.0, endAngle/BezierBoard.DEG_TO_RAD + 90.0,
            // targetAngle/BezierBoard.DEG_TO_RAD +90.0);
            if (startAngle >= targetAngle && endAngle <= targetAngle) {
                // System.out.printf("getLengthByTangentReverseScaled() Value should be in this span, StartAngle:%f EndAngle:%f
                // TargetAngle:%f\n", startAngle/BezierBoard.DEG_TO_RAD +90.0, endAngle/BezierBoard.DEG_TO_RAD + 90.0,
                // targetAngle/BezierBoard.DEG_TO_RAD +90.0);
            }
            ...
            if (endAngle > targetAngle) {
                // System.out.printf("getLengthByTangentReverseScaled() i:%d, endAngle > targetAngle endAngle:%f
                // targetAngle:%f\n",i, endAngle/BezierBoard.DEG_TO_RAD, targetAngle/BezierBoard.DEG_TO_RAD);
                if (useMinimumAngleOnSharpCorners == true) {
                    i += 1;
                    // System.out.printf("getLengthByTangentReverseScaled() Target found by endAngle(%f) > targetAngle(%f) with
                    // use minimun angle on shape corner\n", endAngle/BezierBoard.DEG_TO_RAD+90.0,
                    // targetAngle/BezierBoard.DEG_TO_RAD+90.0);
                } else {
            ...
            // startAngle/BezierBoard.DEG_TO_RAD, endAngle/BezierBoard.DEG_TO_RAD, initial_t, last_t);
            ...
            if (angleError <= ANGLE_TOLERANCE) {
                ...
                // System.out.printf("getLengthByTangentReverseScaled() Value should be in this span but error is too large,
                // StartAngle:%f EndAngle:%f Target: %f Returned: %f t:%f\n", startAngle/BezierBoard.DEG_TO_RAD +90.0,
                // endAngle/BezierBoard.DEG_TO_RAD +90.0, targetAngle/BezierBoard.DEG_TO_RAD +90.0,
        ...
        if (Math.abs(tangent - targetAngle) > 0.5 && targetFound) {
            // System.out.printf("getLengthByTangentReverseScaled() getTangentByCurveLength() returned different angle. Target:
            // %f Returned: %f length:%f curveLength:%f\n", targetAngle/BezierBoard.DEG_TO_RAD +90.0,
            // tangent/BezierBoard.DEG_TO_RAD + 90.0, length, lengthScaled(scaleX, scaleY));
    ...
    public String toString() {
        ...
        for (int i = 0; i < this.getNrOfControlPoints(); i++) {
            if (string.length() > 0) string.concat(";");
