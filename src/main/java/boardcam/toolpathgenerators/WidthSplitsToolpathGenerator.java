...
public class WidthSplitsToolpathGenerator extends AbstractToolpathGenerator {
    ...
    public WidthSplitsToolpathGenerator(
            AbstractCutter cutter,
            AbstractBlankHoldingSystem holdingSystem,
            AbstractMachineWriter writer,
    ...
    public Point3d getToolpathCoordinate() {
        if (mCurrentState == State.STATE_DECK) {
            ...
            if (length == 0) length = mBoard.getLength();
            if (nrOfLengthSplits == 0) nrOfLengthSplits = (length / x_res);
            ...
            y = (mBoard.getWidthAt(x) / 2.0) * ((double) (i % nrOfWidthSplits) / (double) nrOfWidthSplits)
                    * ((i > nrOfWidthSplits) ? -1.0 : 1.0);
