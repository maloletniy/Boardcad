...
public abstract class AbstractToolpathGenerator {
    ...
    AbstractToolpathGenerator(AbstractMachineWriter writer, Component parentComponent) {
        setWriter(writer);

        mProgress = new ProgressMonitor(parentComponent, LanguageResource.getString("GENERATINGTOOLPATH_STR"), "", 0, 100);
    }
    ...
    protected void init() {
        try {
            ...
            System.out.println("Exception in AbstractToolpathGenerator::init(): " + e.toString());
        }
    }
    ...
    protected void writeToolpath() {
        ...
        do {
            ...
            if (retCoord == null) break;

            retVector = getToolpathNormalVector();

            double newSpeed = calcSpeed(retCoord, retVector, mBoard, isAtStringer());
    ...
    public void processCoordinate(Point3d coordinate, Vector3d normal) // throws Throwable TODO: fixme
    {
        ...
        // DEBUG
        // System.out.printf("coordinate: %f,%f,%f normal:
        ...
        finalCoord = mCurrentCutter.calcOffset(transformedCoord, transformedVector, mBoard);

        Point3d finalCoordRaw = new Point3d(finalCoord[0], finalCoord[1], finalCoord[2]);

        Point3d finalCoordTransformed = new Point3d(finalCoord[0], finalCoord[1], finalCoord[2]);
        ...
        // Check collision between cutter and blank holding system, and handle it
        else if (mCurrentBlankHoldingSystem != null
                && mCurrentBlankHoldingSystem.checkCollision(finalCoordRaw, mCurrentCutter) == true) {
            ArrayList<double[]> collisionHandlingToolpath = mCurrentBlankHoldingSystem
                    .handleCollision(finalCoordRaw, mCurrentCutter, mBoard);
            ...
            for (int i = 0; i < collisionHandlingToolpath.size(); i++) {
                ...
                // System.out.printf("Collision coordinate transformed: %f, %f, %f\n", finalCoordTransformed.x,
                // finalCoordTransformed.y, finalCoordTransformed.z);
            ...
            if (mLastToolpathPoint == null) {
                ...
                writeCoordinate(new double[] { finalCoordTransformed.x, finalCoordTransformed.y, finalCoordTransformed.z });
                mLastToolpathPoint = finalCoordTransformed;
            } else if (!mLastToolpathPoint.equals(finalCoordTransformed)) {
                addToolpathLine(finalCoordTransformed);
    ...
    protected void setProgressDone(int percentDone) {
        String progressString = String.format(LanguageResource.getString("TOOLPATHPROGRESS_STR"), percentDone);
    ...
    abstract protected double calcSpeed(Point3d pos, Vector3d normal, AbstractBoard board, boolean isAtStringer);

    protected boolean checkCollision(Point3d pos, AbstractBoard board) {
        return mCurrentCutter.checkCollision(pos, board);
    }

    protected void handleCollision(Point3d point, AbstractCutter cutter, AbstractBoard board) {
    ...
    protected void reset3DView() {
        if (getMachine3DView() != null) getMachine3DView().reset();
    }

    protected void setSurfaceStart(Point3d point) {
        if (getMachine3DView() != null) getMachine3DView().setDeckSurfaceStart(point);
    }

    protected void addSurfaceLine(Point3d point) {
        if (getMachine3DView() != null) getMachine3DView().addDeckSurfaceLine(point);
    }

    protected void addNormal(Point3d point, Vector3d vector) {
        if (getMachine3DView() != null) getMachine3DView().addDeckNormal(point, vector);
    }

    protected void addToolpathLine(Point3d point) {
        if (getMachine3DView() != null) getMachine3DView().addDeckToolpathLine(point);
    }

    protected void setToolpathStart(Point3d point) {
        if (getMachine3DView() != null) getMachine3DView().setDeckToolpathStart(point);
    }
