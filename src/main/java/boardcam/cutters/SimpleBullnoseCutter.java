...
public class SimpleBullnoseCutter extends AbstractCutter {
    ...
    public SimpleBullnoseCutter(MachineConfig config) {
        ...
        cutterSettings.addObject(
                CUTTER_DIAMETER,
                cutterSettings.new Measurement(2.54),
                LanguageResource.getString("CUTTERDIAMETER_STR"),
                cb);
        cutterSettings.addObject(
                CUTTER_CORNERRADIUS,
                cutterSettings.new Measurement(0.5),
                LanguageResource.getString("CUTTERCORNERRADIUS_STR"),
                cb);
        cutterSettings.addObject(
                CUTTER_HEIGHT,
                cutterSettings.new Measurement(6.5),
                LanguageResource.getString("CUTTERHEIGHT_STR"),
                cb);
    }
    ...
    public double[] calcOffset(Point3d pos, Vector3d normal, AbstractBoard board) {
        ...
        if (normalFromAbove.x < 0) headingAngle = -headingAngle;

        // System.out.printf("headingAngle: %f y_angle: %f\n", headingAngle/BezierBoard.DEG_TO_RAD,
        // y_angle/BezierBoard.DEG_TO_RAD);
        ...
        if (Math.abs(y_angle) / MathUtils.DEG_TO_RAD < mConsideredFlatAngle) {
            ...
            if (Math.abs(x_angle) < mConsideredFlatAngle) {
                toolCompensationX = flatAreaRadius
                        * MathUtils.clamp(Math.sin(x_angle) / Math.sin(mConsideredFlatAngle * MathUtils.DEG_TO_RAD), -1.0, 1.0);
            } else {
                toolCompensationX = (mDiameter * 10.0 / 2.0) - mCornerRadius * 10.0 * (1.0 - Math.sin(x_angle));
            }

            double toolCompensationY = flatAreaRadius
                    * MathUtils.clamp(Math.sin(y_angle) / Math.sin(mConsideredFlatAngle * MathUtils.DEG_TO_RAD), -1.0, 1.0);
    ...
    public void update3DModel() {
        ...
        // Spindle
        m3DModelArray.setCoordinates(
                i * SEGMENT_POINTS,
                new Point3d[] { new Point3d(0, 0, mHeight * 10.0), new Point3d(0, 0, (mHeight + 10.0) * 10.0) });
    ...
    public AxisAlignedBoundingBox getBoundingBox(Point3d pos) {
        ...
        return new AxisAlignedBoundingBox(
                new Point3d(pos.x - r, pos.y - r, pos.z),
                new Point3d(pos.x + r, pos.y + r, pos.z + mHeight * UnitUtils.MILLIMETER_PR_CENTIMETER));
    }

    public String toString() {
        return String.format(
                "%s %s:%s, %s:%s, %s:%s, ",
                LanguageResource.getString("BULLNOSECUTTER_STR"),
                LanguageResource.getString("CUTTERDIAMETER_STR"),
                UnitUtils.convertLengthToCurrentUnit(mDiameter, false),
                LanguageResource.getString("CUTTERCORNERRADIUS_STR"),
                UnitUtils.convertLengthToCurrentUnit(mCornerRadius, false),
                LanguageResource.getString("CUTTERHEIGHT_STR"),
                UnitUtils.convertLengthToCurrentUnit(mHeight, false));
    }
}
