...
/**
 * A flat bottom cutter with rounded corners which allows curved rails to be cut without the need for a 4th axis so the 4th axis
 * has not been implemented.
 * 
 * The 2d profile is defined by the flatRadius and cornerRadius parameters. The cut is made by a point contact along the 90 deg
 * arc of the corner, where the input surface normal lines up.
 */
public class Bullnose2dCutter extends AbstractCutter {
    ...
    /**
     * Calculates tool compensation based on the surface normal and adds it to the position. This cutter will always meet the
     * board surface along the corner radius from 0-90 degrees normal.
     *
    ...
    public double[] calcOffset(Point3d pos, Vector3d normal, AbstractBoard board) {
        ...
        double headingAngle = Math.atan2(normalFromAbove.y, normalFromAbove.x) - Math.atan2(leftVec2D.y, leftVec2D.x);// Vector2d.angle()
                                                                                                                      // doesn't
                                                                                                                      // work
                                                                                                                      // because
                                                                                                                      // it
        ...
        /*
         * DEBUG pos.x /= 25.4; pos.y /= 25.4; pos.z /= 25.4; normal.x /= 25.4; normal.y /= 25.4; normal.z /= 25.4;
         * toolCompensation.x /= 25.4; toolCompensation.y /= 25.4; toolCompensation.z /= 25.4; headingAngle *= 180 / Math.PI;
         * cornerAngle *= 180 / Math.PI; hComp /= 25.4; vComp /= 25.4;
         * 
         * System.out.printf("%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f,%.3f\n", pos.x, pos.y, pos.z,
         * normal.x, normal.y, normal.z, toolCompensation.x, toolCompensation.y, toolCompensation.z, headingAngle, cornerAngle,
         * hComp, vComp);
         */
    ...
    /**
     * Adapts method to transform inputs from boardcad coordinates to machine coordinates and calls the other calcOffset().
