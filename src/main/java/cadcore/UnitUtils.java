...
public class UnitUtils {
    ...
    public static double convertInputStringToInternalVolumeUnit(String string) {
        ...
        try {
            string = string.replace(",", ".");
            if (string.endsWith("l") || string.endsWith("liter") || string.endsWith("litre") || string.endsWith("dm^3")
                    || string.endsWith("dm�") || string.endsWith("dm3")) {
                mul = 1; // Default to cubic dm (liter)
            } else if (string.endsWith("m") || string.endsWith("m3") || string.endsWith("m^3") || string.endsWith("m�")
                    || string.endsWith("cubicmeter") || string.endsWith("cubic") || string.endsWith("cubic meter")
                    || string.endsWith("cubicmetre") || string.endsWith("cubic metre")) {
                mul = 1000.0; // to cubic meter
            } else if (string.endsWith("cubic feet") || string.endsWith("cubic foot") || string.endsWith("cubic ft")
                    || string.endsWith("cubic ft") || string.endsWith("cu feet") || string.endsWith("cu foot")
                    || string.endsWith("cu ft") || string.endsWith("ft�") || string.endsWith("ft") || string.endsWith("ft3")
                    || string.endsWith("feet�") || string.endsWith("foot�") || string.endsWith("feet^3")
                    || string.endsWith("foot^3") || string.endsWith("ft^3")) {
                mul = 28.3168466; // Pound pr. cubic foot
            }

            if (string.charAt(string.length() - 1) == '3') string = string.substring(0, string.length() - 1);
    ...
    public static String convertLengthToUnit(double value, boolean useLargeUnits, int unit) {
        ...
        case INCHES: {
            ...
            if (inches >= 1) {
                if (format.length() > 0) format = format.concat(" ");
    ...
    public static String convertAreaToCurrentUnit(double value) {
        ...
        case METERS: {
            return String.format(
                    "%.3f %s",
                    value / (CENTIMETER_PR_METER * CENTIMETER_PR_METER),
                    LanguageResource.getString("UNITSQUAREMETERS_STR"));
        }
    ...
    public static String convertMomentOfInertiaToCurrentUnit(double value) {
        ...
        case INCHES_DECIMAL: {
            return String.format(
                    "%.3f %s",
                    value * UnitUtils.POUNDS_PR_KG * UnitUtils.FEET_PR_METER * UnitUtils.FEET_PR_METER,
                    LanguageResource.getString("UNITPOUNDSPRFOOTSQUARED_STR"));
        }
