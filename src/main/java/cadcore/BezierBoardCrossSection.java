...
public class BezierBoardCrossSection implements Cloneable, Comparable {
    ...
    public double getCenterThickness() {
        return (mCrossSectionSpline.getControlPoint(mCrossSectionSpline.getNrOfControlPoints() - 1).getPoints()[0].y
                - mCrossSectionSpline.getControlPoint(0).getPoints()[0].y);
    }
    ...
    public double getReleaseAngle() {
        Point2D.Double pos = getPointAtS(
                mCrossSectionSpline
                        .getSByNormalReverse(BezierBoardDrawUtil.TUCK_UNDER_DEFINITION_ANGLE * MathUtils.DEG_TO_RAD)); // TODO:
                                                                                                                       // Bad
                                                                                                                       // dependency
    ...
    public double getTuckRadius() {
        double sForApex = mCrossSectionSpline
                .getSByNormalReverse(BezierBoardDrawUtil.APEX_DEFINITION_ANGLE * MathUtils.DEG_TO_RAD); // TODO: Bad dependency
        double sForTuck = mCrossSectionSpline
                .getSByNormalReverse(BezierBoardDrawUtil.TUCK_UNDER_DEFINITION_ANGLE * MathUtils.DEG_TO_RAD); // TODO: Bad
                                                                                                              // dependency
        ...
        for (int i = 0; i < steps; i++) {
            double curvatureAtStep = mCrossSectionSpline.getCurvatureByS(sForApex + i * step);
    ...
    public void scale(double newThickness, double newWidth) {
        ...
        if ((oldThickness * newThicknessScale) <= 0.1) return;

        if ((oldWidth * newWidtScale) <= 0.1) return;
    ...
    public int compareTo(Object other) {
        ...
        if (mPosition == otherCrossSection.mPosition) return 0;
    ...
    public boolean equals(Object other) {
        ...
        if (mPosition != otherCrossSection.mPosition) return false;

        if (mCrossSectionSpline.getNrOfControlPoints() != otherCrossSection.mCrossSectionSpline.getNrOfControlPoints()) {
            return false;
        }

        for (int i = 0; i < mCrossSectionSpline.getNrOfControlPoints(); i++) {
            if (!mCrossSectionSpline.getControlPoint(i).equals(otherCrossSection.mCrossSectionSpline.getControlPoint(i))) {
                return false;
            }
        }
    ...
    public synchronized BezierBoardCrossSection interpolate(BezierBoardCrossSection target, double t) {
        try {
            ...
            interpolationTargetSandboxCopy.scale(getCenterThickness(), getWidth());

            if (interpolationSandboxCopy.mCrossSectionSpline
                    .getNrOfControlPoints() != interpolationTargetSandboxCopy.mCrossSectionSpline.getNrOfControlPoints()) {
                ...
                double scaleX = otherBezier.getMaxX() / mostControlPointsBezier.getMaxX();
                double scaleY = otherBezier.getMaxY() / mostControlPointsBezier.getMaxY();
                ...
                while (mostControlPointsBezier.getNrOfControlPoints() > otherBezier.getNrOfControlPoints()) {
                    ...
                    for (int i = 1; i < mostControlPointsBezier.getNrOfControlPoints() - 1; i++) {
                        BezierKnot currentControlPoint = mostControlPointsBezier.getControlPoint(i);
                        ...
                        for (int j = 1; j < otherBezier.getNrOfControlPoints() - 1; j++) {
                            ...
                            // poor matching
                            BezierKnot otherControlPoint = (BezierKnot) otherBezier.getControlPoint(j).clone();
                            otherControlPoint.setControlPointLocation(
                                    otherControlPoint.getEndPoint().x * scaleX,
                                    otherControlPoint.getEndPoint().y * scaleY);

                            double currentMatch = currentControlPoint.compareTo(otherControlPoint);
                    ...
                    int index = otherBezier.getSplitControlPoint(worstMatchControlPoint.getPoints()[0], newControlPoint);
                    ...
                    if (index > 0) {
                        ...
                        BezierKnot prev = otherBezier.getControlPoint(index - 1);
                        BezierKnot next = otherBezier.getControlPoint(index + 1);
            ...
            for (int i = 0; i < interpolationTargetSandboxCopy.mCrossSectionSpline.getNrOfControlPoints(); i++) {
                BezierKnot a = interpolationSandboxCopy.mCrossSectionSpline.getControlPoint(i);
                BezierKnot b = interpolationTargetSandboxCopy.mCrossSectionSpline.getControlPoint(i);
                BezierKnot v = interpolated.mCrossSectionSpline.getControlPoint(i);

                // Calculate vectors between ControlPoints
                for (int j = 0; j < 3; j++) {
                    VecMath.subVector(a.getPoints()[j], b.getPoints()[j], v.getPoints()[j]);
                    VecMath.scaleVector(v.getPoints()[j], t);
                    VecMath.addVector(a.getPoints()[j], v.getPoints()[j], v.getPoints()[j]);
                }
    ...
    public Object clone() {
        ...
        try {
            ...
            System.out.println("Exception in BezierBoardCrossSection::clone(): " + e.toString());
            throw new Error("CloneNotSupportedException in CrossSection");
        }

        crossSection.mCrossSectionSpline = (BezierSpline) this.mCrossSectionSpline.clone();
