...
class BezierBoardLinearInterpolationSurfaceModel extends AbstractBezierBoardSurfaceModel {
    ...
    synchronized public double getCrosssectionAreaAt(final BezierBoard brd, final double x, int splits) {
        ...
        final MathUtils.Function deckFunc = new MathUtils.Function() {

            public double f(double y) {
                return getDeckAt(brd, x, y).z;
            }
        };

        double deckIntegral = MathUtils.Integral.getIntegral(deckFunc, a, b, splits);

        final MathUtils.Function bottomFunc = new MathUtils.Function() {

            public double f(double y) {
                return getBottomAt(brd, x, y).z;
            }
        };
