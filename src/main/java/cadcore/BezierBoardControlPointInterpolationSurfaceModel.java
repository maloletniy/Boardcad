...
class BezierBoardControlPointInterpolationSurfaceModel extends AbstractBezierBoardSurfaceModel {
    ...
    synchronized public Point3d getPointAt(
        ...
        if (x > brd.getLength() - 0.1) x = brd.getLength() - 0.1;
        ...
        if (minAngle > 0.0) {
            minS = crossSection.getBezierSpline()
                    .getSByNormalReverse(minAngle * MathUtils.DEG_TO_RAD, useMinimumAngleOnSharpCorners);
        }

        if (maxAngle < 270.0) {
            maxS = crossSection.getBezierSpline()
                    .getSByNormalReverse(maxAngle * MathUtils.DEG_TO_RAD, useMinimumAngleOnSharpCorners);
        }

        if (minS > BezierSpline.ONE) minS = BezierSpline.ONE;
        if (maxS < BezierSpline.ZERO) maxS = BezierSpline.ZERO;
    ...
    synchronized public Vector3d getNormalAt(
            final BezierBoard brd,
            double x,
            double s,
            double minAngle,
        ...
        BezierBoardCrossSection crossSection = (BezierBoardCrossSection) (brd.getInterpolatedCrossSection(x).clone());
        if (crossSection == null) return new Vector3d(0.0, 0.0, 0.0);
    ...
    synchronized public double getCrosssectionAreaAt(final BezierBoard brd, final double x, int splits) {
        final BezierBoardCrossSection crossSection = brd.getInterpolatedCrossSection(x);
        if (crossSection == null) return 0.0;

        double ttAtRail = crossSection.getBezierSpline().getTTByNormal(90.0 * MathUtils.DEG_TO_RAD);
        ...
        double deckIntegral = MathUtils.Integral.getIntegral(func, ttAtRail, 1.0, splits);

        double bottomIntegral = MathUtils.Integral.getIntegral(func, 0.0, ttAtRail, splits);
