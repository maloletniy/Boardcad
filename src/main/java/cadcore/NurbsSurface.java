...
//=========================================================NurbsSurface
/**
 * This class implements a NURBS surface, which is used for creating a 3D model of the board. The class can also represent
 * special cases of nurbs surfaces such as bezier patches and b-spline surfaces.
 */
...
public class NurbsSurface implements Cloneable {
    ...
    /**
     * Set the knot values to bspline representation, i.e. uniform distances between each knot value.
     */
    ...
    /**
     * Set the knot values to bezier representation, i.e. having three points with the same value
     */
    ...
    /**
     * Decides if the surface will be evaluated at each point or approximated from a triangulated surface. Evaluating the
     * surface is slow, but more accurate.
     *
                    ...
                    for (int j = 0; j < points_per_segment; j++) {
                        for (double v = 0.0; v < 1.0 - epsilon; v = v + step) {
                            evaluated_points[(int) (((i + u) / step) + epsilon)][(int) (((j + v) / step)
                                    + epsilon)] = evaluate_point_on_surface(i, j, u, v);
                        }
                    }
                }
            }
    ...
    /**
     * Gets a point on the surface. For a bspline surface the position is always calculated directly by evaluating the surface
     * function. For bezier and full nurbs surfaces the point is either estimated from the mesh or evaluated directly depending
     * on the whether always_evaluate is set.
     *
     ...
     */
    @Deprecated
    public NurbsPoint get_point_on_surface(int segment, int point, double u, double v) {
        ...
        if (ep == null) {
            ...
            System.out.println(
                    "matrix1=" + (int) (((segment + u) / step) + epsilon) + " matrix2="
                            + (int) (((point + v) / step) + epsilon));
            return evaluate_point_on_surface(segment, point, u, v);
        }
     ...
     */
    @Deprecated
    private NurbsPoint evaluate_point_on_surface(int segment, int point, double u2, double v2) {
        ...
        if (surface_type == 2) return get_point_on_surface_old(segment, point, u2, v2);
    ...
    /**
     * Gets a point on the surface. The surface is parameterized using it's knot vectors. The parameters t and s defines a
     * position on the surface. Use the functions {@link #getMinT()}, {@link #getMaxT()}, {@link #getMinS()}, and
    ...
    private double n(int i, int k, double t) {
        ...
        if (u[i + k] > u[i]) a = (t - u[i]) / (u[i + k] - u[i]);

        if (u[i + k + 1] > u[i + 1]) b = (u[i + k + 1] - t) / (u[i + k + 1] - u[i + 1]);
    ...
    private double n2(int i, int k, double t) {
        ...
        if (v[i + k] > v[i]) a = (t - v[i]) / (v[i + k] - v[i]);

        if (v[i + k + 1] > v[i + 1]) b = (v[i + k + 1] - t) / (v[i + k + 1] - v[i + 1]);
    ...
    /**
     * Gets a point on the surface using the old algorithms for bspline surfaces. These are very quick, but does not work for
     * other surface types.
     *
     * @param segment
     * @param point
     ...
     */
    @Deprecated
    public NurbsPoint get_point_on_surface_old(int segment, int point, double u, double v) {
    ...
    private double b(double t) {
        ...
        return 0.16666666666666666666666666666667 + 0.5 * (-one_sub_at * (one_sub_at * one_sub_at - one_sub_at - 1)); // optimized
                                                                                                                      // slightly
                                                                                                                      // more
    }

    /**
     * Gets the curvate at a given point on the surface. This function should updated to use the new surface parameterization...
     *
     * @param segment
     * @param point
     * @param u
     ...
     */
    @Deprecated
    public Vector3f calculate_normal(int i, int j, double myu, double myv, boolean bottom) {
        ...
        if (s1 < u[0]) s1 = u[0];
        if (s2 > u[nr_of_segments - 1]) s2 = u[nr_of_segments - 1];
        if (t1 < v[0]) t1 = v[0];
        if (t2 > v[points_per_segment - 1]) t2 = v[points_per_segment - 1];
    ...
    /**
     * Calculates the surface normal at a position given by the parameters s and t
     *
     * @param s
     * @param t
    ...
    /**
     * Gets the surface point at a position given by the parameters t and s
     *
     * @param s
     * @param t
