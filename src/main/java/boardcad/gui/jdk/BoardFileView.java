...
public class BoardFileView extends FileView {
    ...
    Icon paintBoard(BufferedImage image, BezierBoard brd, String type) {
        ...
        // BezierBoardDrawUtil.paintBezierPaths(new JavaDraw(g), leftMargin, height-topMargin, scale , Color.BLACK, new
        // BasicStroke(1.0f/(float)scale), new BezierSpline[]{brd.getDeck(), brd.getBottom()}, BezierBoardDrawUtil.Vertical |
        // BezierBoardDrawUtil.FlipY);
        // BezierBoardDrawUtil.paintBezierPaths(new JavaDraw(g), image.getWidth()/2.0, height-topMargin, scale, Color.BLACK, new
        // BasicStroke(1.0f/(float)scale), new BezierSpline[]{brd.getOutline()}, BezierBoardDrawUtil.Vertical |
        // BezierBoardDrawUtil.FlipY | BezierBoardDrawUtil.MirrorX);
        ...
        BezierBoardDrawUtil.paintBezierSplines(
                ...
                BezierBoardDrawUtil.FlipY | BezierBoardDrawUtil.MirrorY,
                true);
        // g.setStroke(new BasicStroke(1.0f));
        // jd.draw(new Line2D.Double(leftMargin, topMargin + brd.getMaxWidth()*scale, width-leftMargin, topMargin +
        // brd.getMaxWidth()*scale));
        // jd.draw(new Line2D.Double(leftMargin + brd.getMaxWidthPos()*scale, topMargin + brd.getMaxWidth()*0.5*scale,
        // leftMargin + brd.getMaxWidthPos()*scale, topMargin + brd.getMaxWidth()*1.5*scale));
        BezierBoardDrawUtil.paintBezierSplines(
                jd,
                leftMargin,
                outlinePos + (brd.getMaxWidth() / 2.0 + brd.getThickness() + 10.0) * scale,
                scale,
                Color.BLACK,
                stroke,
                ...
                BezierBoardDrawUtil.FlipY,
                true);
        // BezierBoardDrawUtil.paintFins(jd, leftMargin, topMargin + brd.getMaxWidth()*scale, scale, Color.BLACK, stroke,
        // brd.mFins, false, false);
