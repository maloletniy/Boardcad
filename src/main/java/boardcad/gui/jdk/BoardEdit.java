...
public class BoardEdit extends JComponent implements AbstractEditor, MouseInputListener, MouseWheelListener {
    ...
    ArrayList<BezierKnot> mSelectedControlPoints = new ArrayList<BezierKnot>();
    ...
    public void setLifeSize(boolean lifeSize) {
        if (mIsLifeSize == lifeSize) return;
    ...
    protected BezierBoard getCurrentBrd() {
    ...
    public void fitBrd() {
        ...
        mOffsetX = noseToTheLeft
                ? getCurrentBrd().getLength() * mScale + (BORDER * dim.width / 100)
    ...
    public void addSelectedControlPoint(BezierKnot selected) {
        if (!mSelectedControlPoints.contains(selected)) mSelectedControlPoints.add(selected);
    }
    ...
    public Point2D.Double screenCoordinateToBrdCoordinate(Point scrPoint) {
        ...
        brdPoint.x = ((double) scrPoint.x - mOffsetX) / mScale
                * ((mDrawControl & BezierBoardDrawUtil.FlipX) != 0 ? -1.0f : 1.0f);
        brdPoint.y = ((double) scrPoint.y - mOffsetY) / mScale
    ...
    public void adjustBackgroundImageTail(Point clickedScreenCoord) {
        ...
        // System.out.printf("adjustBackgroundImageTail() rot:%f mulY:%f mBackgroundImageRot: %f, mBackgroundImageOffsetX: %f,
        // mBackgroundImageOffsetY: %f, mBackgroundImageScale: %f", rot, mulY, mBackgroundImageRot, mBackgroundImageOffsetX,
        // mBackgroundImageOffsetY, mBackgroundImageScale);
    ...
    public void adjustBackgroundImageNose(Point clickedScreenCoord) {
        ...
        // System.out.printf("adjustBackgroundImageNose() rot:%f mulY:%f mBackgroundImageRot: %f, mBackgroundImageOffsetX: %f,
        // mBackgroundImageOffsetY: %f, mBackgroundImageScale: %f\n", rot, mulY, mBackgroundImageRot, mBackgroundImageOffsetX,
        // mBackgroundImageOffsetY, mBackgroundImageScale);
    ...
    Point2D.Double getNose() {
        return (Point2D.Double) getActiveBezierSplines(getCurrentBrd())[0]
                .getControlPoint(getActiveBezierSplines(getCurrentBrd())[0].getNrOfControlPoints() - 1).getEndPoint().clone();
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        if (mDrawControl == 0) {
            ...
            bh.draw_outline3D(
                    ...
                    (int) mOffsetY,
                    false,
                    BoardCAD.getInstance().mViewBottomCut.isSelected(),
                    BoardCAD.getInstance().mViewDeckCut.isSelected(),
                    mRotationMatrix);
            ...
            BoardCAD.getInstance().status_panel.set_mode(
                    BoardCAD.getInstance().getCurrentCommand().getCommandString() + " "
                            + LanguageResource.getString("MODE_STR"));
        ...
        if (!BoardCAD.getInstance().mViewBlank.isSelected()) {
            ...
            if (BoardCAD.getInstance().isPaintingGrid()) {
                BezierBoardDrawUtil.paintGrid(
                        new JavaDraw(g2d),
                        (double) mOffsetX,
                        (double) mOffsetY,
                        (double) mScale,
                        BoardCAD.getInstance().getGridColor(),
                        currentBrd.getLength(),
                        currentBrd.getMaxWidth() / 2.0,
            ...
            if (BoardCAD.getInstance().mGhostMode && ghostBrd.isEmpty() == false) {
                if (BoardCAD.getInstance().isPaintingOriginalBrd()) {
                    if (orgBrd != null && getActiveBezierSplines(orgBrd) != null) {
                        AffineTransform savedTransform = BezierBoardDrawUtil
                                .setTransform(new JavaDraw(g2d), mOriginalOffsetX * mScale, mOriginalOffsetY * mScale, 1.0);
                        drawPart(
                                g2d,
                                BoardCAD.getInstance().getOriginalBrdColor(),
                                stroke,
                                orgBrd,
                                BoardCAD.getInstance().useFill());
                ...
                if (BoardCAD.getInstance().isPaintingCurvature()) {
                    drawCurvature(
                            g2d,
                            BoardCAD.getInstance().getGhostBrdColor(),
                            new BasicStroke((float) (BoardCAD.getInstance().getCurvatureThickness() / mScale)),
                            currentBrd);
                }

                if (isPaintingVolumeDistribution()) {
                    drawVolumeDistribution(
                            g2d,
                            BoardCAD.getInstance().getGhostBrdColor(),
                            new BasicStroke((float) (BoardCAD.getInstance().getVolumeDistributionThickness() / mScale)),
                            currentBrd);
                }
                ...
                if (BoardCAD.getInstance().isPaintingCenterOfMass()) {
                    ...
                    drawCircle(
                            g2d,
                            BoardCAD.getInstance().getCenterOfMassColor(),
                            new BasicStroke(
                                    (float) (BoardCAD.getInstance().getSelectedControlPointOutlineThickness() / mScale)),
                            new Point2D.Double(
                                    x * Math.cos(mGhostRot) + (mGhostOffsetX * mScale),
                                    x * Math.sin(mGhostRot) + (mGhostOffsetY * mScale)));
                }
                if (ghostBrd != null && getActiveBezierSplines(ghostBrd) != null) {
                    AffineTransform savedTransform = BezierBoardDrawUtil
                            .setTransform(new JavaDraw(g2d), mGhostOffsetX * mScale, mGhostOffsetY * mScale, mGhostRot, 1.0);
                    ...
                    if (BoardCAD.getInstance().isPaintingCurvature()) {
                        drawCurvature(
                                g2d,
                                BoardCAD.getInstance().getCurvatureColor(),
                                new BasicStroke((float) (BoardCAD.getInstance().getCurvatureThickness() / mScale)),
                                ghostBrd);
                    }
                    ...
                    if (isPaintingVolumeDistribution()) {
                        drawVolumeDistribution(
                                g2d,
                                BoardCAD.getInstance().getVolumeDistributionColor(),
                                new BasicStroke((float) (BoardCAD.getInstance().getVolumeDistributionThickness() / mScale)),
                                ghostBrd);
                    }
                    ...
                    if (BoardCAD.getInstance().isPaintingControlPoints()) {
                        drawControlPoints(
                                g2d,
                                new BasicStroke(
                                        (float) (BoardCAD.getInstance().getUnselectedControlPointOutlineThickness() / mScale)),
                                ghostBrd);
                    }
                ...
                if (BoardCAD.getInstance().isPaintingGhostBrd()) {
                    if (ghostBrd != null && getActiveBezierSplines(ghostBrd) != null) {
                        AffineTransform savedTransform = BezierBoardDrawUtil.setTransform(
                                new JavaDraw(g2d),
                                mGhostOffsetX * mScale,
                                mGhostOffsetY * mScale,
                                mGhostRot,
                                1.0);
                        drawPart(
                                g2d,
        ...
        if (mDrawControl == BezierBoardDrawUtil.MirrorY) {
            bh.draw_outline(
                    ...
                    BoardCAD.getInstance().mViewBottomCut.isSelected(),
                    BoardCAD.getInstance().mViewDeckCut.isSelected(),
                    BoardCAD.getInstance().mViewBlank.isSelected());
    ...
    public void drawCircle(Graphics2D g, Color color, Stroke stroke, Point2D.Double pos) {
        ...
        BezierBoardDrawUtil.paintCircle(
                new JavaDraw(g),
                mOffsetX,
                mOffsetY,
                mScale,
    ...
    public void drawControlPoints(Graphics2D g, Stroke stroke, BezierBoard brd) {
        BezierBoardDrawUtil.paintBezierControlPoints(
                new JavaDraw(g),
                mOffsetX,
                mOffsetY,
                mScale,
    ...
    public void drawCurvature(Graphics2D g, Color color, Stroke stroke, BezierBoard brd) {
        BezierBoardDrawUtil.paintCurvature(
                new JavaDraw(g),
                mOffsetX,
                mOffsetY,
                mScale,
    ...
    public void drawVolumeDistribution(Graphics2D g, Color color, Stroke stroke, BezierBoard brd) {
        BezierBoardDrawUtil.paintVolumeDistribution(
                new JavaDraw(g),
                mOffsetX,
                mOffsetY,
                mScale,
...
class QuadView extends JPanel implements BrdEditParentContainer {
    ...
    public BoardEdit getActive() {
        if (mActive == null) return (BoardEdit) getComponent(0);
