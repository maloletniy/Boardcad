...
public class SrfReader {
    ...
    static public int loadFile(BezierBoard brd, String aFilename) {
        ...
        try {
            ...
            for (i = nrOfPointsOutline - 1; i >= 0; i--) {
                ...
                controlPoint.setEndPoint(
                        boardLength - outline[i * 3].x * UnitUtils.CENTIMETER_PR_METER,
                        outline[i * 3].y * UnitUtils.CENTIMETER_PR_METER);
                controlPoint.setTangentToNext(
                        boardLength - outline[(i * 3) + 2].x * UnitUtils.CENTIMETER_PR_METER,
                        outline[(i * 3) + 2].y * UnitUtils.CENTIMETER_PR_METER);
                controlPoint.setTangentToPrev(
                        boardLength - outline[(i * 3) + 1].x * UnitUtils.CENTIMETER_PR_METER,
                        outline[(i * 3) + 1].y * UnitUtils.CENTIMETER_PR_METER);
            ...
            for (i = nrOfPointsRocker - 1; i >= 0; i--) {
                controlPoint = new BezierKnot();
                controlPoint.setEndPoint(
                        boardLength - rocker[i * 3].x * UnitUtils.CENTIMETER_PR_METER,
                        rocker[i * 3].y * UnitUtils.CENTIMETER_PR_METER);
                controlPoint.setTangentToNext(
                        boardLength - rocker[(i * 3) + 2].x * UnitUtils.CENTIMETER_PR_METER,
                        rocker[(i * 3) + 2].y * UnitUtils.CENTIMETER_PR_METER);
                controlPoint.setTangentToPrev(
                        boardLength - rocker[(i * 3) + 1].x * UnitUtils.CENTIMETER_PR_METER,
                        rocker[(i * 3) + 1].y * UnitUtils.CENTIMETER_PR_METER);
            ...
            for (i = nrOfPointsRocker - 1; i >= 0; i--) {
                controlPoint = new BezierKnot();
                controlPoint.setEndPoint(
                        boardLength - deck[i * 3].x * UnitUtils.CENTIMETER_PR_METER,
                        deck[i * 3].y * UnitUtils.CENTIMETER_PR_METER);
                controlPoint.setTangentToNext(
                        boardLength - deck[(i * 3) + 2].x * UnitUtils.CENTIMETER_PR_METER,
                        deck[(i * 3) + 2].y * UnitUtils.CENTIMETER_PR_METER);
                controlPoint.setTangentToPrev(
                        boardLength - deck[(i * 3) + 1].x * UnitUtils.CENTIMETER_PR_METER,
                        deck[(i * 3) + 1].y * UnitUtils.CENTIMETER_PR_METER);
            ...
            for (i = nrOfPointsRail - 1; i >= 0; i--) {
                controlPoint = new BezierKnot();
                controlPoint.setEndPoint(
                        boardLength - rail[i * 3].x * UnitUtils.CENTIMETER_PR_METER,
                        rail[i * 3].y * UnitUtils.CENTIMETER_PR_METER);
                controlPoint.setTangentToNext(
                        boardLength - rail[(i * 3) + 2].x * UnitUtils.CENTIMETER_PR_METER,
                        rail[(i * 3) + 2].y * UnitUtils.CENTIMETER_PR_METER);
                controlPoint.setTangentToPrev(
                        boardLength - rail[(i * 3) + 1].x * UnitUtils.CENTIMETER_PR_METER,
                        rail[(i * 3) + 1].y * UnitUtils.CENTIMETER_PR_METER);
            ...
            for (i = nrOfPointsBottom - 1; i >= 0; i--) {
                controlPoint = new BezierKnot();
                controlPoint.setEndPoint(
                        boardLength - bottom[i * 3].x * UnitUtils.CENTIMETER_PR_METER,
                        bottom[i * 3].y * UnitUtils.CENTIMETER_PR_METER);
                controlPoint.setTangentToNext(
                        boardLength - bottom[(i * 3) + 2].x * UnitUtils.CENTIMETER_PR_METER,
                        bottom[(i * 3) + 2].y * UnitUtils.CENTIMETER_PR_METER);
                controlPoint.setTangentToPrev(
                        boardLength - bottom[(i * 3) + 1].x * UnitUtils.CENTIMETER_PR_METER,
                        bottom[(i * 3) + 1].y * UnitUtils.CENTIMETER_PR_METER);
            ...
            for (i = 0; i < nrOfDeckCaves; i++) {
                ...
                if (crsPos > boardLength - 0.5) crsPos = boardLength - 0.5;
                ...
                for (int j = 0; j < nrOfBottomCaves; j++) {
                    if (Math.abs(
                        ...
                        controlPoint.setEndPoint(
                                bottomCavePoints[j * 4].x * UnitUtils.CENTIMETER_PR_METER,
                                bottomCavePoints[j * 4].y * UnitUtils.CENTIMETER_PR_METER - crsBottom);
                        controlPoint.setTangentToNext(
                                bottomCavePoints[(j * 4) + 1].x * UnitUtils.CENTIMETER_PR_METER,
                                bottomCavePoints[(j * 4) + 1].y * UnitUtils.CENTIMETER_PR_METER - crsBottom);
                        ...
                        controlPoint.setContinous(false);
                        controlPoint.setTangentToPrev(
                                bottomCavePoints[(j * 4) + 3].x * UnitUtils.CENTIMETER_PR_METER,
                                bottomCavePoints[(j * 4) + 3].y * UnitUtils.CENTIMETER_PR_METER - crsBottom);
                        controlPoint.setEndPoint(
                                bottomCavePoints[(j * 4) + 2].x * UnitUtils.CENTIMETER_PR_METER,
                                bottomCavePoints[(j * 4) + 2].y * UnitUtils.CENTIMETER_PR_METER - crsBottom);
                        controlPoint.setTangentToNext(
                                bottomCavePoints[(j * 4) + 2].x * UnitUtils.CENTIMETER_PR_METER,
                                bottomCavePoints[(j * 4) + 2].y * UnitUtils.CENTIMETER_PR_METER - crsBottom + 0.2);
                ...
                if (!found) {
                    ...
                    if (Double.isInfinite(r) || Double.isNaN(r)) r = 1.0;

                    // Get center contol points from bottom cave
                    double beforeCenterTangentX = bottomCavePoints[(caveBefore * 4) + 1].x * UnitUtils.CENTIMETER_PR_METER
                            - bottomCavePoints[caveBefore * 4].x * UnitUtils.CENTIMETER_PR_METER;
                    double beforeCenterTangentY = bottomCavePoints[(caveBefore * 4) + 1].y * UnitUtils.CENTIMETER_PR_METER
                            - bottomCavePoints[caveBefore * 4].y * UnitUtils.CENTIMETER_PR_METER;

                    double afterCenterTangentX = bottomCavePoints[(caveAfter * 4) + 1].x * UnitUtils.CENTIMETER_PR_METER
                            - bottomCavePoints[caveAfter * 4].x * UnitUtils.CENTIMETER_PR_METER;
                    double afterCenterTangentY = bottomCavePoints[(caveAfter * 4) + 1].y * UnitUtils.CENTIMETER_PR_METER
                            - bottomCavePoints[caveAfter * 4].y * UnitUtils.CENTIMETER_PR_METER;
                    ...
                    double beforeRailTangentX = bottomCavePoints[(caveBefore * 4) + 3].x * UnitUtils.CENTIMETER_PR_METER
                            - bottomCavePoints[(caveBefore * 4) + 2].x * UnitUtils.CENTIMETER_PR_METER;
                    double beforeRailTangentY = bottomCavePoints[(caveBefore * 4) + 3].y * UnitUtils.CENTIMETER_PR_METER
                            - bottomCavePoints[(caveBefore * 4) + 2].y * UnitUtils.CENTIMETER_PR_METER;

                    double afterRailTangentX = bottomCavePoints[(caveAfter * 4) + 3].x * UnitUtils.CENTIMETER_PR_METER
                            - bottomCavePoints[(caveAfter * 4) + 2].x * UnitUtils.CENTIMETER_PR_METER;
                    double afterRailTangentY = bottomCavePoints[(caveAfter * 4) + 3].y * UnitUtils.CENTIMETER_PR_METER
                            - bottomCavePoints[(caveAfter * 4) + 2].y * UnitUtils.CENTIMETER_PR_METER;
                    ...
                    controlPoint.setTangentToPrev(
                            railTangentX + controlPoint.getEndPoint().x,
                            railTangentY + controlPoint.getEndPoint().y);
                ...
                controlPoint.setContinous(false);
                controlPoint.setEndPoint(
                        deckCavePoints[(i * 4) + 2].x * UnitUtils.CENTIMETER_PR_METER,
                        deckCavePoints[(i * 4) + 2].y * UnitUtils.CENTIMETER_PR_METER - crsBottom);
                controlPoint.setTangentToPrev(
                        deckCavePoints[(i * 4) + 2].x * UnitUtils.CENTIMETER_PR_METER,
                        deckCavePoints[(i * 4) + 2].y * UnitUtils.CENTIMETER_PR_METER - crsBottom - 0.2);
                controlPoint.setTangentToNext(
                        deckCavePoints[(i * 4) + 3].x * UnitUtils.CENTIMETER_PR_METER,
                        deckCavePoints[(i * 4) + 3].y * UnitUtils.CENTIMETER_PR_METER - crsBottom);
                ...
                controlPoint.setContinous(false);
                controlPoint.setTangentToPrev(
                        deckCavePoints[(i * 4) + 1].x * UnitUtils.CENTIMETER_PR_METER,
                        deckCavePoints[(i * 4) + 1].y * UnitUtils.CENTIMETER_PR_METER - crsBottom);
                controlPoint.setEndPoint(
                        deckCavePoints[i * 4].x * UnitUtils.CENTIMETER_PR_METER,
                        deckCavePoints[i * 4].y * UnitUtils.CENTIMETER_PR_METER - crsBottom);
            ...
            for (i = 0; i < nrOfBottomCaves; i++) {
                ...
                controlPoint.setContinous(false);
                controlPoint.setEndPoint(
                        bottomCavePoints[i * 4].x * UnitUtils.CENTIMETER_PR_METER,
                        bottomCavePoints[i * 4].y * UnitUtils.CENTIMETER_PR_METER - crsBottom);
                controlPoint.setTangentToNext(
                        bottomCavePoints[(i * 4) + 1].x * UnitUtils.CENTIMETER_PR_METER,
                        bottomCavePoints[(i * 4) + 1].y * UnitUtils.CENTIMETER_PR_METER - crsBottom);
                ...
                controlPoint.setContinous(false);
                controlPoint.setTangentToPrev(
                        bottomCavePoints[(i * 4) + 3].x * UnitUtils.CENTIMETER_PR_METER,
                        bottomCavePoints[(i * 4) + 3].y * UnitUtils.CENTIMETER_PR_METER - crsBottom);
                controlPoint.setEndPoint(
                        bottomCavePoints[(i * 4) + 2].x * UnitUtils.CENTIMETER_PR_METER,
                        bottomCavePoints[(i * 4) + 2].y * UnitUtils.CENTIMETER_PR_METER - crsBottom);
                controlPoint.setTangentToNext(
                        bottomCavePoints[(i * 4) + 2].x * UnitUtils.CENTIMETER_PR_METER,
                        bottomCavePoints[(i * 4) + 2].y * UnitUtils.CENTIMETER_PR_METER - crsBottom + 0.2);
                ...
                // Get center contol points from bottom cave
                double beforeCenterTangentX = deckCavePoints[(caveBefore * 4) + 3].x * UnitUtils.CENTIMETER_PR_METER
                        - deckCavePoints[(caveBefore * 4) + 2].x * UnitUtils.CENTIMETER_PR_METER;
                double beforeCenterTangentY = deckCavePoints[(caveBefore * 4) + 3].y * UnitUtils.CENTIMETER_PR_METER
                        - deckCavePoints[(caveBefore * 4) + 2].y * UnitUtils.CENTIMETER_PR_METER;

                double afterCenterTangentX = deckCavePoints[(caveAfter * 4) + 3].x * UnitUtils.CENTIMETER_PR_METER
                        - deckCavePoints[(caveAfter * 4) + 2].x * UnitUtils.CENTIMETER_PR_METER;
                double afterCenterTangentY = deckCavePoints[(caveAfter * 4) + 3].y * UnitUtils.CENTIMETER_PR_METER
                        - deckCavePoints[(caveAfter * 4) + 2].y * UnitUtils.CENTIMETER_PR_METER;
                ...
                controlPoint.setEndPoint(
                        crs.getBezierSpline().getControlPoint(1).getEndPoint().x,
                        railBezier.getValueAt(railPos) - crsBottom);
                controlPoint.setTangentToPrev(controlPoint.getEndPoint().x, controlPoint.getEndPoint().y - 0.2);
                controlPoint.setTangentToNext(
                        centerTangentX + controlPoint.getEndPoint().x,
                        centerTangentY + controlPoint.getEndPoint().y);

                crs.getBezierSpline().append(controlPoint);

                // Get rail contol points from bottom cave
                double beforeRailTangentX = deckCavePoints[(caveBefore * 4) + 1].x * UnitUtils.CENTIMETER_PR_METER
                        - deckCavePoints[(caveBefore * 4)].x * UnitUtils.CENTIMETER_PR_METER;
                double beforeRailTangentY = deckCavePoints[(caveBefore * 4) + 1].y * UnitUtils.CENTIMETER_PR_METER
                        - deckCavePoints[(caveBefore * 4)].y * UnitUtils.CENTIMETER_PR_METER;

                double afterRailTangentX = deckCavePoints[(caveAfter * 4) + 1].x * UnitUtils.CENTIMETER_PR_METER
                        - deckCavePoints[(caveAfter * 4)].x * UnitUtils.CENTIMETER_PR_METER;
                double afterRailTangentY = deckCavePoints[(caveAfter * 4) + 1].y * UnitUtils.CENTIMETER_PR_METER
                        - deckCavePoints[(caveAfter * 4)].y * UnitUtils.CENTIMETER_PR_METER;
    ...
    Point2D.Double[] getDeckCaveAtPos(
            double pos,
            double targetWidth,
            double targetHeight,
            double deckCaves,
            Point3d[] deckCavesPoints) {
