...
public class BezierBoard extends AbstractBoard implements Cloneable {
    ...
    protected double[] mFins = new double[9]; // x, y for back of fin, x,y for front of fin, bac of center, front of center,
                                              // depth of center, depth of sidefin, splay angle
    ...
    public void reset() {
        ...
        mFins = new double[9]; // x, y for back of fin, x,y for front of fin, bac of center, front of center, depth of center,
                               // depth of sidefin, splay angle
        mFinType = new String();
    ...
    public BezierBoardCrossSection getCurrentCrossSection() {
        if (mCurrentCrossSection < 1 || mCurrentCrossSection > mCrossSections.size() - 2) return null;
    ...
    public void nextCrossSection() {
        if (++mCurrentCrossSection >= mCrossSections.size() - 1) mCurrentCrossSection = 1;
    }

    public void previousCrossSection() {

        if (--mCurrentCrossSection <= 0) mCurrentCrossSection = mCrossSections.size() - 2;
    }
    ...
    public int getPreviousCrossSectionIndex(double pos) {
        ...
        if (index > getCrossSections().size() - 2) index = getCrossSections().size();
    ...
    public int getNextCrossSectionIndex(double pos) {
        ...
        if (index > getCrossSections().size() - 2) index = getCrossSections().size() - 2;
    ...
    public double getDeckAtPos(double x, double y) {
        ...
        /*
         * //Calculate scales double widthAtPos = getWidthAtPos(x); double thicknessAtPos = getThicknessAtPos(x);
         *
         * //Get the position from function since we cheat with the crosssections at tip and tail double pos1 =
         * getPreviousCrossSectionPos(x); double pos2 = getNextCrossSectionPos(x);
         *
         * //Get crosssections but use the first and last real crosssections if we're at the dummy crosssections at nose and
         * tail BezierBoardCrossSection c1 = getPreviousCrossSection(x); BezierBoardCrossSection c2 = getNextCrossSection(x);
         *
         * //Get scales and values double Scale1Y = c1.getWidth() / widthAtPos; double Scale1Z = c1.getCenterThickness()/
         * thicknessAtPos;
         *
         * double Scale2Y = c2.getWidth() / widthAtPos; double Scale2Z = c2.getCenterThickness()/ thicknessAtPos;
         *
         * double v1 = c1.getDeckAtPos(y*Scale1Y); double v2 = c2.getDeckAtPos(y*Scale2Y);
         *
         * v1 /= Scale1Z; v2 /= Scale2Z;
         *
         * //Get blended point double p = (x - pos1)/(pos2 - pos1);
         *
    ...
    public double getBottomAtPos(double x, double y) {
        return AbstractBezierBoardSurfaceModel.getBezierBoardSurfaceModel(getInterpolationType())
                .getBottomAt(this, x, y)
                .getZ();
    ...
    public Point2D.Double getSurfacePointAtPos(double x, double s) {
        Point3d pos = AbstractBezierBoardSurfaceModel.getBezierBoardSurfaceModel(getInterpolationType())
                .getPointAt(this, x, s, -90.0, 360.0, true);
    ...
    public Point3d getSurfacePoint(
        ...
        Point3d point = AbstractBezierBoardSurfaceModel.getBezierBoardSurfaceModel(getInterpolationType()).getPointAt(
                this,
                x,
                (double) currentSplit / (double) totalSplits,
                minAngle,
                maxAngle,
    ...
    public Point3d getSurfacePoint(double x, double s) {
        ...
        point = AbstractBezierBoardSurfaceModel.getBezierBoardSurfaceModel(getInterpolationType())
                .getPointAt(this, x, s, -360.0, 360.0, true);
        return point;
    ...
    public Vector3d getSurfaceNormal(
        ...
        Vector3d normal = AbstractBezierBoardSurfaceModel.getBezierBoardSurfaceModel(getInterpolationType()).getNormalAt(
                this,
                x,
                (double) currentSplit / (double) totalSplits,
                minAngle,
    ...
    public Vector3d getSurfaceNormal(double x, double s) {
        Vector3d normal = AbstractBezierBoardSurfaceModel.getBezierBoardSurfaceModel(getInterpolationType())
                .getNormalAt(this, x, s, -360.0, 360.0, true);
        return normal;
    }
    ...
    public Vector3d getTangentAtPos(double x, double sa, double sb) {
        ...
        // Get crosssections but use the first and last real crosssections position if we're at the dummy crosssections at nose
        // and tail
    ...
    public BezierBoardCrossSection getInterpolatedCrossSection(double x) {
        if (getCrossSections().size() == 0) return null;

        if (x < 0) return null;

        if (x > getLength()) return null;
    ...
    public double getArea() {
        final MathUtils.Function f = new MathUtils.Function() {
    ...
    public double getVolume() {
        if (getCrossSections().size() < 3) return 0;

        double a = 0.01;

        double b = getLength() - 0.01;

        MathUtils.Function crsSecAreaFunc = new MathUtils.Function() {

            public double f(double x) {
                return getCrossSectionAreaAt(x, VOLUME_X_SPLITS);
            }
        };
    ...
    public double getCenterOfMass() {
        if (mCenterOfMass != 0) return mCenterOfMass;

        double momentSum = 0.0;
        double weightSum = 0.0;

        if (getCrossSections().size() < 3) return 0.0;
    ...
    public double getCrossSectionAreaAt(final double pos, final int splits) {
        double area = AbstractBezierBoardSurfaceModel.getBezierBoardSurfaceModel(getInterpolationType())
                .getCrosssectionAreaAt(this, pos, splits);
    ...
    public double getDeckAt(double x, double y) {
        if (getWidthAt(x) / 2.0 < y) return 0.0;
        ...
        Point3d point = AbstractBezierBoardSurfaceModel.getBezierBoardSurfaceModel(getInterpolationType())
                .getDeckAt(this, x, y);
        return point.z;
    ...
    public void scaleAccordingly(double newLength, double newWidth, double newThickness) {
        ...
        for (int i = 1; i < mDeckSpline.getNrOfControlPoints() - 1; i++) {
            ...
            // thickness, thicknessScale, targetThickness, actualThickness, dy);
    ...
    public void setLocks() {
        if (mOutlineSpline.getNrOfControlPoints() < 2) return;
        ...
        for (int i = 0; i < mCrossSections.size(); i++) {
            ...
            mCrossSections.get(i)
                    .getBezierSpline()
                    .getControlPoint(mCrossSections.get(i).getBezierSpline().getNrOfControlPoints() - 1)
                    .setMask(0, 0);
        }

        // Set slaves
        mDeckSpline.getControlPoint(0).setSlave(mBottomSpline.getControlPoint(0));
        mDeckSpline.getControlPoint(mDeckSpline.getNrOfControlPoints() - 1)
                .setSlave(mBottomSpline.getControlPoint(mBottomSpline.getNrOfControlPoints() - 1));

        mBottomSpline.getControlPoint(0).setSlave(mDeckSpline.getControlPoint(0));
        mBottomSpline.getControlPoint(mBottomSpline.getNrOfControlPoints() - 1)
                .setSlave(mDeckSpline.getControlPoint(mDeckSpline.getNrOfControlPoints() - 1));
        ...
        for (int i = 0; i < mCrossSections.size(); i++) {
            ...
            mCrossSections.get(i)
                    .getBezierSpline()
                    .getControlPoint(mCrossSections.get(i).getBezierSpline().getNrOfControlPoints() - 1)
                    .setTangentToPrevLocks(BezierKnot.LOCK_X_MORE);
        }
    }
    ...
    boolean deckCollisionTest(Point3d aabbCenter, double width, double depth, double height) {
        if (aabbCenter.z - (height / 2.0) > getMaxRocker()) return false;

        if (aabbCenter.y - (width / 2.0) > getMaxWidth()) return false;

        if (aabbCenter.x - (depth / 2.0) > getLength()) return false;
        ...
        for (int i = 0; i <= depthSplits; i++) {
            ...
            for (int j = 0; i <= widthSplits; i++) {
                ...
                if (z < aabbCenter.y - (width / 2.0)) return true;
    ...
    public NurbsPoint getOutline3D(double x) {
        ...
        /*
         * int ni = getNearestCrossSectionIndex(x); double nearestPos = mCrossSections.get(ni).getPosition(); if(nearestPos < x)
         * { before_index=ni; before_x=nearestPos; after_index=ni+1; after_x= mCrossSections.get(ni+1).getPosition(); } else {
         * before_index=ni-1; before_x=mCrossSections.get(ni-1).getPosition(); after_index=ni; after_x= nearestPos;
         * 
         * }
         */
    ...
    public NurbsPoint getTop3D(double x, double z) {
        ...
        /*
         * ArrayList<BezierControlPoint> top_before_ControlPoints = new ArrayList<BezierControlPoint>(); for(int i =
         * before_ControlPoints.size()-1; i>=before_outline_index; i--) { ControlPoint
         * myControlPoint=(BezierControlPoint)before_ControlPoints.get(i).clone(); myControlPoint.switch_tangents();
        ...
        /*
         * ArrayList<BezierControlPoint> top_after_ControlPoints = new ArrayList<BezierControlPoint>(); for(int i =
         * after_ControlPoints.size()-1; i>=after_outline_index; i--) { ControlPoint
         * myControlPoint=(BezierControlPoint)after_ControlPoints.get(i).clone(); myControlPoint.switch_tangents();
    ...
    public void update3DModel(Shape3D model) {
        ...
        for (int i = 0; i < deckSteps; i++) {
            ...
            // TODO: temporary fix because the control point surface model doesn't work well near stringer. Remove when surface
            // model works better
            if (i == 0) {
            ...
            for (int j = 1; j <= lengthSteps; j++) {
                // TODO: temporary fix because the control point surface model doesn't work well near stringer. Remove when
                // surface model works better
                if (i == 0) {
        ...
        /*
         * minAngle = maxAngle; maxAngle = 174.0; for(int i = 0; i < railSteps; i++) { xPos = 0.0;
         * 
         * //first coords in lengthwise strip vertices[0].set(getSurfacePoint(xPos, minAngle, maxAngle, i, railSteps));
         * normals[0].set(getSurfaceNormal(xPos, minAngle, maxAngle, i, railSteps));
         * 
         * vertices[3].set(getSurfacePoint(xPos, minAngle, maxAngle, i+1, railSteps)); normals[3].set(getSurfaceNormal(xPos,
         * minAngle, maxAngle, i+1, railSteps));
         * 
         * xPos += lengthStep;
         * 
         * for(int j = 1; j <= lengthSteps; j++) { //Two next coords vertices[1].set(getSurfacePoint(xPos, minAngle, maxAngle,
         * i, railSteps)); normals[1].set(getSurfaceNormal(xPos, minAngle, maxAngle, i, railSteps));
         * 
         * vertices[2].set(getSurfacePoint(xPos, minAngle, maxAngle, i+1, railSteps)); normals[2].set(getSurfaceNormal(xPos,
         * minAngle, maxAngle, i+1, railSteps));
         * 
         * if(j == lengthSteps/2) { for(int h = 0; h<1; h++) {
         * System.out.printf("update3DModel() on rails: normals[%d] %f,%f,%f\n", h, normals[h].x, normals[h].y, normals[h].z); }
         * }
         * 
         * //Build one quad quads.setCoordinates(nrOfQuads*4, vertices); quads.setNormals(nrOfQuads*4, normals); nrOfQuads++;
         * 
         * //Mirror for(int n = 0; n < 4; n++) { mirrorVertices[n].set(vertices[3-n]); mirrorNormals[n].set(normals[3-n]);
         * 
         * mirrorVertices[n].y = -mirrorVertices[n].y; mirrorNormals[n].y = -mirrorNormals[n].y; }
         * 
         * //Build mirrored quad quads.setCoordinates(nrOfQuads*4, mirrorVertices); quads.setNormals(nrOfQuads*4,
         * mirrorNormals); nrOfQuads++;
         * 
         * //Get ready for next step vertices[0].set(vertices[1]); normals[0].set(normals[1]);
         * 
         * vertices[3].set(vertices[2]); normals[3].set(normals[2]);
         * 
         * xPos += lengthStep;
         * 
         * } }
    ...
    public String toString() {
        ...
        String measurementsString = LanguageResource.getString("MEASUREMENTS_STR")
                + UnitUtils.convertLengthToCurrentUnit(getLength(), true) + LanguageResource.getString("BY_STR")
                + UnitUtils.convertLengthToCurrentUnit(getCenterWidth(), true) + LanguageResource.getString("BY_STR")
                + UnitUtils.convertLengthToCurrentUnit(getThickness(), true);
