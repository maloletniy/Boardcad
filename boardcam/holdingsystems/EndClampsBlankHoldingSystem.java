...
public class EndClampsBlankHoldingSystem extends AbstractBlankHoldingSystem {
    ...
    public EndClampsBlankHoldingSystem(MachineConfig config) {
        super.init();
        mConfig = config;
        System.out.printf("EndClampsBlankHoldingSystem mConfig set to %s in %s\n", mConfig.toString(), this.toString());
        ...
        supportsSettings.addMeasurement(CLAMP_LENGTH, 80, LanguageResource.getString("CLAMPLENGTH_STR"), cb);
        supportsSettings.addMeasurement(CLAMP_HEIGHT, 20, LanguageResource.getString("CLAMPHEIGHT_STR"), cb);
        supportsSettings.addMeasurement(CLAMP_WIDTH, 20, LanguageResource.getString("CLAMPWIDTH_STR"), cb);
        supportsSettings.addMeasurement(CLAMP_SAFEZONE, 5, LanguageResource.getString("CLAMPSAFEZONE_STR"), cb);

        supportsSettings.addMeasurement(CLAMP_OVERLAP, 8, LanguageResource.getString("CLAMPOVERLAP_STR"), cb);
        supportsSettings.addMeasurement(BLANK_VERTICAL_OFFSET, 10, LanguageResource.getString("BLANKVERTICALOFFSET_STR"), cb);
        ...
        Settings generalSettings = mConfig.getCategory(LanguageResource.getString("GENERALCATEGORY_STR"));
        Setting blankSetting = generalSettings.getSetting(LanguageResource.getString("BLANK_STR"));
    ...
    public void calcBlankOffset() {
        ...
        double clampOverlap = holdingSystemSettings.getMeasurement(CLAMP_OVERLAP);
        double blankVertOffset = holdingSystemSettings.getMeasurement(BLANK_VERTICAL_OFFSET);
        double clampSafeZone = holdingSystemSettings.getMeasurement(CLAMP_SAFEZONE);
        ...
        if (blank != null && !blank.isEmpty()) {
            ...
            double noseBottom = blank.getBottomAt(blank.getLength() - BezierSpline.ZERO, 0.0);

            double rotAngle = Math.atan2(noseMaxPos.y - tailMaxPos.y, noseMaxPos.x - tailMaxPos.x);

            mBlankDeckRotation = rotAngle;
            mBlankBottomRotation = -rotAngle;

            double vertOffset = Math.cos(rotAngle) * tailMaxPos.y - Math.sin(rotAngle) * tailMaxPos.x;
            vertOffset -= blankVertOffset;

            double horOffset = -Math.sin(rotAngle) * tailMaxPos.y;

            mBlankDeckOffset = new Vector3d(horOffset - clampOverlap, 0, -vertOffset);
            mBlankBottomOffset = new Vector3d(horOffset - clampOverlap, 0, vertOffset);

            mRotatedLength = (Math.sin(rotAngle) * (noseBottom - tailBottom)) + (Math.cos(rotAngle) * blank.getLength());
        }
    }
    ...
    public ArrayList<double[]> handleCollision(Point3d point, AbstractCutter cutter, AbstractBoard board) {
        ...
        if (mPastNoseClamp || mPastTailClamp) {
            ...
            String holdingSystemStr = LanguageResource.getString("BLANKHOLDINGSYSTEMCATEGORY_STR");
            Settings holdingSystemSettings = mConfig.getCategory(holdingSystemStr);
            double clampOverlap = holdingSystemSettings.getMeasurement(CLAMP_OVERLAP) * UnitUtils.MILLIMETER_PR_CENTIMETER;
            double clampSafeZone = holdingSystemSettings.getMeasurement(CLAMP_SAFEZONE) * UnitUtils.MILLIMETER_PR_CENTIMETER;

            double clampEdgeX = mCollideWithNoseClamp ? clampSafeZone : mRotatedLength * 10 - clampOverlap * 2 - clampSafeZone; // NOTE:
                                                                                                                                // Tail
                                                                                                                                // clamp
                                                                                                                                // and
                                                                                                                                // nose
                                                                                                                                ...
                                                                                                                                // mixed
                                                                                                                                // up
            Vector3d normal = new Vector3d(mCollideWithNoseClamp ? 1.0 : -1.0, 0.0, 0.0);
            ...
            System.out.printf("--- Collision occured, limiting position to collisionpoint: %f,%f,%f\n", pos.x, pos.y, pos.z);
            ...
            if (!mPreviousClampCollision && mPreviousPos != null) {
                // Straight up from last good pos
                res.add(new double[] { mPreviousPos.x, mPreviousPos.y, calculated[2] });
            }
            ...
            mPreviousPos = new Point3d(calculated[0], calculated[1], calculated[2]);
    ...
    public void draw(Graphics2D g2d, double offsetX, double offsetY, double scale, boolean showDeck) {
        ...
        tailClampRect.setFrame(-clampLength, -clampHeight / 2, clampLength, clampHeight);

        Rectangle2D noseClampRect = new Rectangle2D.Double();
        noseClampRect.setFrame(mRotatedLength - clampOverlap * 2, -clampHeight / 2, clampLength, clampHeight);

        // Draw safe zone
        Rectangle2D tailSafeZoneRect = new Rectangle2D.Double();
        tailSafeZoneRect.setFrame(
                -clampLength,
                -clampHeight / 2 - clampSafeZone,
                clampLength + clampSafeZone,
                clampHeight + clampSafeZone * 2);

        Rectangle2D noseSafeZoneRect = new Rectangle2D.Double();
        noseSafeZoneRect.setFrame(
                mRotatedLength - clampOverlap * 2 - clampSafeZone,
                -clampHeight / 2 - clampSafeZone,
                clampLength + clampSafeZone,
                clampHeight + clampSafeZone * 2);
        ...
        // Draw offset line
        line.setLine(
                tailClampRect.getMaxX(),
                showDeck ? -blankVertOffset : blankVertOffset,
                noseClampRect.getMinX(),
                showDeck ? -blankVertOffset : blankVertOffset);
    ...
    void init3DModel() {
        ...
        mNoseClampBox = new IndexedQuadArray(8, GeometryArray.COORDINATES, 6 * 4);
        mTailClampBox = new IndexedQuadArray(8, GeometryArray.COORDINATES, 6 * 4);
    ...
    public void update3DModel() {
        String holdingSystemStr = LanguageResource.getString("BLANKHOLDINGSYSTEMCATEGORY_STR");
        Settings holdingSystemSettings = mConfig.getCategory(holdingSystemStr);
        double clampLength = holdingSystemSettings.getMeasurement(CLAMP_LENGTH) * UnitUtils.MILLIMETER_PR_CENTIMETER;
        double clampHeight = holdingSystemSettings.getMeasurement(CLAMP_HEIGHT) * UnitUtils.MILLIMETER_PR_CENTIMETER;
        double clampWidth = holdingSystemSettings.getMeasurement(CLAMP_WIDTH) * UnitUtils.MILLIMETER_PR_CENTIMETER;
        double clampOverlap = holdingSystemSettings.getMeasurement(CLAMP_OVERLAP) * UnitUtils.MILLIMETER_PR_CENTIMETER;

        mNoseClampBox.setCoordinate(0, new Point3d(-clampLength, clampWidth / 2.0, clampHeight / 2));
        mNoseClampBox.setCoordinate(1, new Point3d(-clampLength, clampWidth / 2.0, -clampHeight / 2));
        mNoseClampBox.setCoordinate(2, new Point3d(-clampLength, -clampWidth / 2.0, -clampHeight / 2));
        mNoseClampBox.setCoordinate(3, new Point3d(-clampLength, -clampWidth / 2.0, clampHeight / 2));

        mNoseClampBox.setCoordinate(4, new Point3d(0, clampWidth / 2.0, clampHeight / 2));
        mNoseClampBox.setCoordinate(5, new Point3d(0, clampWidth / 2.0, -clampHeight / 2));
        mNoseClampBox.setCoordinate(6, new Point3d(0, -clampWidth / 2.0, -clampHeight / 2));
        mNoseClampBox.setCoordinate(7, new Point3d(0, -clampWidth / 2.0, clampHeight / 2));

        mTailClampBox.setCoordinate(0, new Point3d(mRotatedLength * 10 - clampOverlap * 2, clampWidth / 2.0, clampHeight / 2));
        mTailClampBox.setCoordinate(1, new Point3d(mRotatedLength * 10 - clampOverlap * 2, clampWidth / 2.0, -clampHeight / 2));
        mTailClampBox
                .setCoordinate(2, new Point3d(mRotatedLength * 10 - clampOverlap * 2, -clampWidth / 2.0, -clampHeight / 2));
        mTailClampBox.setCoordinate(3, new Point3d(mRotatedLength * 10 - clampOverlap * 2, -clampWidth / 2.0, clampHeight / 2));

        mTailClampBox.setCoordinate(
                4,
                new Point3d(mRotatedLength * 10 + clampLength - clampOverlap * 2, clampWidth / 2.0, clampHeight / 2));
        mTailClampBox.setCoordinate(
                5,
                new Point3d(mRotatedLength * 10 + clampLength - clampOverlap * 2, clampWidth / 2.0, -clampHeight / 2));
        mTailClampBox.setCoordinate(
    ...
    public void updateBoundingBoxes() {
        ...
        mNoseBox = new AxisAlignedBoundingBox(
                new Point3d(-clampLength, -clampWidth / 2.0 - clampSafeZone, -clampHeight / 2 - clampSafeZone),
                new Point3d(0 + clampSafeZone, clampWidth / 2.0 + clampSafeZone, clampHeight / 2 + clampSafeZone));
        mTailBox = new AxisAlignedBoundingBox(
                new Point3d(
                        mRotatedLength * 10 - clampOverlap * 2 - clampSafeZone,
                        -clampWidth / 2.0 - clampSafeZone,
                        -clampHeight / 2 - clampSafeZone),
                new Point3d(
                        mRotatedLength * 10 - clampOverlap * 2 - clampSafeZone + clampLength,
                        clampWidth / 2.0 + clampSafeZone,
                        clampHeight / 2 + clampSafeZone));

        mPastNoseBox = new AxisAlignedBoundingBox(
                new Point3d(-Double.MAX_VALUE, -Double.MAX_VALUE, -Double.MAX_VALUE),
                new Point3d((-clampLength), Double.MAX_VALUE, Double.MAX_VALUE));
        mPastTailBox = new AxisAlignedBoundingBox(
                new Point3d((mRotatedLength * 10 + clampLength - clampOverlap * 2), -Double.MAX_VALUE, -Double.MAX_VALUE),
                new Point3d(Double.MAX_VALUE, Double.MAX_VALUE, Double.MAX_VALUE));
    }
    ...
    public String toString(boolean isDeckCut) {
        ...
        String details = String.format(
                ...
                LanguageResource.getString("CLAMPLENGTH_STR"),
                clampLength,
                LanguageResource.getString("CLAMPHEIGHT_STR"),
                clampHeight,
                LanguageResource.getString("CLAMPWIDTH_STR"),
                clampWidth,
                LanguageResource.getString("CLAMPSAFEZONE_STR"),
                clampSafeZone,
                LanguageResource.getString("CLAMPOVERLAP_STR"),
                clampOverlap,
