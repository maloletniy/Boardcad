...
public class SupportsBlankHoldingSystem extends AbstractBlankHoldingSystem {
    ...
    void calcBlankDeckOffset() {
        ...
        if (blank == null || blank.isEmpty()) return;
        ...
        double distanceToSupport2 = holdingSystemSettings.getDouble(SUPPORT_2_POS) / UnitUtils.MILLIMETER_PR_CENTIMETER
                - holdingSystemSettings.getDouble(TAILSTOP_POS) / UnitUtils.MILLIMETER_PR_CENTIMETER;
        ...
        for (int i = 0; i < 10; i++) {
            ...
            // Vector2d blankVector = new Vector2d(blankDistanceToSupport2-blankDistanceToSupport1,
            // blankSupport2Y-blankSupport1Y);
            ...
            // lineYOffset);
            Point2D.Double blankSupport1Pos = bottomSpline.getPointByLineIntersection(
                    new Point2D.Double(blankDistanceToSupport1, 0),
                    new Point2D.Double(blankDistanceToSupport1 - lineXOffset, lineYOffset));
            Point2D.Double blankSupport2Pos = bottomSpline.getPointByLineIntersection(
                    new Point2D.Double(blankDistanceToSupport2, 0),
                    new Point2D.Double(blankDistanceToSupport2 - lineXOffset, lineYOffset));
            Vector2d blankVector2 = new Vector2d(
                    blankSupport2Pos.x - blankSupport1Pos.x,
                    blankSupport2Pos.y - blankSupport1Pos.y);
            double blankAngle2 = blankVector2.angle(new Vector2d(1, 0))
                    * ((blankSupport1Pos.y >= blankSupport2Pos.y) ? 1.0 : -1.0);

            // System.out.printf("calcBlankDeckOffset() iteration: %d blankDistanceToSupport1: %f blankDistanceToSupport2:
    ...
    void calcApproxBoardDeckOffset() {
        ...
        if (mBoard == null || mBoard.isEmpty()) return;
        ...
        double blankTailPointYAfterRot = Math.cos(mBlankDeckRotation) * blankTailPoint
                - Math.sin(mBlankDeckRotation) * tailPickPos;

        double blankNosePoint = blank.getDeck().getValueAt(nosePickPos);
        double blankNosePointYAfterRot = Math.cos(mBlankDeckRotation) * blankNosePoint
                - Math.sin(mBlankDeckRotation) * nosePickPos;
        Point2D.Double blankVec = new Point2D.Double(
                nosePickPos - tailPickPos,
                blankNosePointYAfterRot - blankTailPointYAfterRot);
        ...
        double boardTailPointYAfterRot = Math.cos(mBoardDeckRotation) * mBoardTailPoint
                - Math.sin(mBoardDeckRotation) * tailPickPos;
    ...
    void calcBoardAndBlankBottomOffset() {
        ...
        for (int i = 0; i < 5; i++) {
            double blankTailX = Math.min(
                    (boardSpaceDeckDiffX + boardSpaceBlankTailDeckOffsetX) * Math.cos(totalAngle)
                            + (boardSpaceDeckDiffY - boardSpaceBlankTailDeckOffsetY) * Math.sin(totalAngle),
                    (boardSpaceDeckDiffX + boardSpaceBlankTailBottomOffsetX) * Math.cos(totalAngle)
                            + (boardSpaceDeckDiffY - boardSpaceBlankTailBottomOffsetY) * Math.sin(totalAngle));
            ...
            // System.out.printf("calcBoardAndBlankBottomOffset() iteration: %d mBoardDistanceToSupport1:%f
            // mBoardDistanceToSupport2: %f\n", i, boardDistanceToSupport1, boardDistanceToSupport2);
            ...
            //
            // Vector2d mBoardVector = new Vector2d(boardDistanceToSupport2-boardDistanceToSupport1,
            // boardSupport2Y-boardSupport1Y);
            ...
            Point2D.Double boardSupport1Pos = deckSpline.getPointByLineIntersection(
                    new Point2D.Double(boardDistanceToSupport1, 0),
                    new Point2D.Double(boardDistanceToSupport1 + lineXOffset, lineYOffset));
            Point2D.Double boardSupport2Pos = deckSpline.getPointByLineIntersection(
                    new Point2D.Double(boardDistanceToSupport2, 0),
                    new Point2D.Double(boardDistanceToSupport2 + lineXOffset, lineYOffset));
            Vector2d boardVector2 = new Vector2d(
                    boardSupport2Pos.x - boardSupport1Pos.x,
                    boardSupport2Pos.y - boardSupport1Pos.y);
            double boardAngle2 = boardVector2.angle(new Vector2d(1, 0))
                    * ((boardSupport1Pos.y >= boardSupport2Pos.y) ? 1.0 : -1.0);

            // System.out.printf("calcBoardAndBlankBottomOffset() iteration: %d boardDistanceToSupport1: %f
            // boardDistanceToSupport2: %f\n", i, boardDistanceToSupport1, boardDistanceToSupport2);
            // System.out.printf("calcBoardAndBlankBottomOffset() iteration: %d boardSupport1Y: %f boardSupport2Y: %f\n", i,
            // boardSupport1Y, boardSupport2Y);
            // System.out.printf("calcBoardAndBlankBottomOffset() iteration: %d boardAngle: %f\n", i, mBoardAngle);
            // System.out.printf("calcBoardAndBlankBottomOffset() iteration: %d boardSupport1Pos.x: %f boardSupport1Pos.y:
            // %f\n", i, boardSupport1Pos.x, boardSupport1Pos.y);
            // System.out.printf("calcBoardAndBlankBottomOffset() iteration: %d boardSupport2Pos.x: %f boardSupport2Pos.y:
            ...
            // System.out.printf("supportsAngle: %f - mBoardAngle: %f = totalAngle %f\n", supportsAngle*180.0/Math.PI,
            // mBoardAngle*180.0/Math.PI, totalAngle*180.0/Math.PI);

            Ox = -blankTailX;
            Oy = -support1Height - (boardDistanceToSupport1 * Math.sin(totalAngle))
                    - (boardSupport1Pos.y / Math.cos(totalAngle)); // OK
        ...
        mBlankBottomOffset.z = -Oy + (boardSpaceDeckDiffX * Math.sin(totalAngle))
                + (boardSpaceDeckDiffY * Math.cos(totalAngle)); // OK
        mBlankBottomRotation = mBoardBottomRotation - deckDiffRotation;

        // System.out.printf("BOTTOM BRD x:%f y:%f rot:%f BLANK x:%f y:%f rot:%f\n", mBoardBottooffsetX, mBoardBottooffsetY,
        // mBoardBottomRotation, mBlankBottooffsetX, mBlankBottooffsetY, mBlankBottomRotation);
    }
    ...
    public void draw(Graphics2D g2d, double offsetX, double offsetY, double scale, boolean showDeck) {
        ...
        // if(showDeck)
        // {
        // jd.draw(new Line2D.Double(offsetX+(mBoardDeckOffset.x*scale), offsetY+(mBoardDeckOffset.z*scale),
        // offsetX+(mBoardDeckOffset.x*scale) + 250.0*Math.cos(mBoardDeckRotation)*scale, offsetY+(mBoardDeckOffset.z*scale) +
        // 250.0*Math.sin(mBoardDeckRotation)*scale));
        //
        // jd.draw(new Line2D.Double(offsetX+(mBlankDeckOffset.x*scale), offsetY+(mBlankDeckOffset.z*scale),
        // offsetX+(mBlankDeckOffset.x*scale) + 250.0*Math.cos(mBlankDeckRotation)*scale, offsetY+(mBlankDeckOffset.z*scale) +
        // 250.0*Math.sin(mBlankDeckRotation)*scale));
        // }
        // else{
        // jd.draw(new Line2D.Double(offsetX+(mBoardBottomOffset.x*scale), offsetY+(mBoardBottomOffset.z*scale),
        // offsetX+(mBoardBottomOffset.x*scale) + 250.0*Math.cos(mBoardBottomRotation)*scale,
        // offsetY+(mBoardBottomOffset.z*scale) + 250.0*Math.sin(mBoardBottomRotation)*scale));
        //
        // jd.draw(new Line2D.Double(offsetX+(mBlankBottomOffset.x*scale), offsetY+(mBlankBottomOffset.z*scale),
        // offsetX+(mBlankBottomOffset.x*scale) + 250.0*Math.cos(mBlankBottomRotation)*scale,
        // offsetY+(mBlankBottomOffset.z*scale) + 250.0*Math.sin(mBlankBottomRotation)*scale));
        //
        // }
    }
    ...
    void init3DModel() {
        ...
        mSupportStructureArray.setCoordinates(
                0,
                new double[] {
                        0,
                        0,
    ...
    public void update3DModel() {
        ...
        // deckTranslation.x -= (BoardCAD.getInstance().getCurrentBrd().getLength() +
        // (mConfig.getCategory(holdingSystemStr).getDouble(TAILSTOP_POS)/UnitUtils.MILLIMETER_PR_CENTIMETER))/2.0;
