    ...
    public void paintComponent(Graphics g) {
        ...
        if (brd != null) {
            BezierBoardDrawUtil.paintBezierSpline(
                    jd,
                    mOffsetX + (boardOffset.x * mScale),
                    mOffsetY - (boardOffset.z * mScale),
                    boardAngle,
                    mScale,
                    BoardCAD.getInstance().getBrdColor(),
                    stroke,
                    brd.getDeck(),
                    (mShowDeck ? BezierBoardDrawUtil.FlipY : 0),
                    false);
            BezierBoardDrawUtil.paintBezierSpline(
                    jd,
                    mOffsetX + (boardOffset.x * mScale),
        ...
        if (blank != null && !blank.isEmpty()) {
            ...
            BezierBoardDrawUtil.paintBezierSpline(
                    jd,
                    mOffsetX + (blankOffset.x * mScale),
                    mOffsetY - (blankOffset.z * mScale),
                    blankAngle,
                    mScale,
                    blankColor,
                    stroke,
