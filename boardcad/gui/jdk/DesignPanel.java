...
class DesignPanel extends Panel implements AbstractEditor {
    ...
    public DesignPanel(BoardHandler board_handler, StatusPanel sp) {
         ...
         * 
         * add(rocker_view); add(edge_view); add(outline_view); add(threed_view);
         * 
         * doLayout(); redraw(); // threed_view.setSize(getPreferredSize()); threed_view.doLayout();
         */
...
class EdgeView extends View implements ItemListener, ActionListener {
    ...
    MouseMotionListener EdgeMouseMotion = new MouseMotionAdapter() {
        ...
        public void mouseDragged(MouseEvent e) {
            if (e.isMetaDown()) {
                ...
                is_dragged = true;
                if (is_marked) {
                    if (!y_locked_item.getState()) board_handler.set_y((-e.getY() + offset_y) / scale);
                    if (!z_locked_item.getState()) board_handler.set_z((e.getX() - offset_x) / scale);
                    ...
                    if (dragged_y > clicked_y)
                        dragged_y = clicked_y + (int) (Math.abs(dragged_x - clicked_x) * (double) off_dimension.height
                                / (double) off_dimension.width);
                    else
                        dragged_y = clicked_y - (int) (Math.abs(dragged_x - clicked_x) * (double) off_dimension.height
                                / (double) off_dimension.width);
        ...
        public void mouseMoved(MouseEvent e) {
            if (!is_marked) design_panel
                    .set_coordinates(board_handler.get_x(), (-e.getY() + offset_y) / scale, (e.getX() - offset_x) / scale);
        }
    };
    ...
    public void update(Graphics g2) {
        ...
        g2d.setRenderingHint(
                ...
                BoardCAD.getInstance().isAntialiasing()
...
class OutlineView extends View implements ItemListener, ActionListener {
    ...
    MouseMotionListener OutlineMouseMotion = new MouseMotionAdapter() {
        ...
        public void mouseDragged(MouseEvent e) {
            if (e.isMetaDown()) {
                ...
                is_dragged = true;
                if (is_marked) {
                    if (!x_locked_item.getState()) board_handler.set_x((e.getX() - offset_x) / scale);
                    if (!z_locked_item.getState()) board_handler.set_z((e.getY() - offset_y) / scale);
                    board_handler.set_point();
                    design_panel.set_coordinates(board_handler.get_x(), board_handler.get_y(), board_handler.get_z());
                    design_panel.redraw();
                } else {
        ...
        public void mouseMoved(MouseEvent e) {
            if (!is_marked) design_panel
                    .set_coordinates((e.getX() - offset_x) / scale, board_handler.get_y(), (e.getY() - offset_y) / scale);
        }
    };
    ...
    public void update(Graphics g2) {
        ...
        board_handler.draw_outline(
                g2d,
                off_dimension.width,
                off_dimension.height,
                scale,
                offset_x,
                offset_y,
                t_under_item.getState(),
                bottom_cut_item.getState(),
                deck_cut_item.getState(),
                false);

        if (blank_item.getState()) board_handler.draw_blank_outline(
                g2d,
                off_dimension.width,
                off_dimension.height,
                scale,
...
class RockerView extends View implements ItemListener, ActionListener {
    ...
    MouseMotionListener RockerMouseMotion = new MouseMotionAdapter() {
        ...
        public void mouseDragged(MouseEvent e) {
            if (e.isMetaDown()) {
                ...
                if (is_marked) {
                    if (!x_locked_item.getState()) board_handler.set_x((e.getX() - offset_x) / scale);
                    if (!y_locked_item.getState()) board_handler.set_y((offset_y - e.getY()) / scale);
                    board_handler.set_point();
                    design_panel.set_coordinates(board_handler.get_x(), board_handler.get_y(), board_handler.get_z());
                    design_panel.redraw();
                } else {
        ...
        public void mouseMoved(MouseEvent e) {
            if (!is_marked) design_panel
                    .set_coordinates((e.getX() - offset_x) / scale, (offset_y - e.getY()) / scale, board_handler.get_z());
        }
    };
    private CheckboxMenuItem blank_item;
    private MenuItem fit_item;
    ...
    public void update(Graphics g2) {
        ...
        if (blank_item.getState()) board_handler.draw_blank_rocker(
                g2d,
                off_dimension.width,
                off_dimension.height,
                scale,
...
class ThreeDView extends Panel implements ItemListener {
    ...
    public ThreeDView(BoardHandler bh, DesignPanel dp) {
        ...
        /*
         * viewingPlatform.addRotateBehavior(0); viewingPlatform.addZoomBehavior(1); viewingPlatform.addTranslateBehavior(2);
         */
    ...
    private BranchGroup createSceneGraph() {
        ...
        /*
         * //add texture java.net.URL texImage=null; try { texImage = new java.net.URL("file:airbrush.jpg"); // texImage = new
         * java.net.URL("file:"+filename); } catch (java.net.MalformedURLException ex) { System.out.println(ex.getMessage());
         * System.exit(1); } // Set up the texture map TextureLoader tex = new TextureLoader(texImage, this);
         * look.setTexture(tex.getTexture());
         *
         * // TextureAttributes texAttr = new TextureAttributes(); // texAttr.setTextureMode(TextureAttributes.MODULATE); //
         * look.setTextureAttributes(texAttr);
         *
         */
    ...
    class ThreeDMouse extends MouseAdapter {
        ...
        public void mouseClicked(MouseEvent e) {
            if (e.isMetaDown()) view_menu.show(the_view, e.getX(), e.getY());
        }
    }
        ...
        public void mouseDragged(MouseEvent e) {
            if (e.isMetaDown()) {
                ...
                if (is_marked) {
                    ...
                    board_handler.set_x(
                            m[0][0] * (e.getX() - offset_x) / scale + m[0][1] * myy + m[0][2] * (e.getY() - offset_y) / scale);// +
                                                                                                                               // m[0][2]*100);
                    board_handler.set_y(
                            m[1][0] * (e.getX() - offset_x) / scale + m[1][1] * myy + m[1][2] * (e.getY() - offset_y) / scale);// +
                                                                                                                               // m[1][2]*100);
                    board_handler.set_z(
                            m[2][0] * (e.getX() - offset_x) / scale + m[2][1] * myy + m[2][2] * (e.getY() - offset_y) / scale);// +
                                                                                                                               // m[2][2]*100);
            ...
            if (e.isMetaDown()) {
                ...
                if (is_dragged) {
                    ...
                    if (is_marked) {
                         ...
                         * if(!x_locked_item.getState()) board_handler.set_x((e.getX()-offset_x)/scale);
                         * if(!z_locked_item.getState()) board_handler.set_z((e.getY()-offset_y)/scale);
                         * board_handler.set_point(rotation_matrix); is_marked=board_handler.outline_mark(scale,
                         * rotation_matrix); design_panel.redraw();
                         */
                    } else {
                        if (clicked_x < dragged_x)
                            zoom_x = clicked_x;
