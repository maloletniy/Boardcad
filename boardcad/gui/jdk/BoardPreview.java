...
public final class BoardPreview extends JComponent implements PropertyChangeListener {
    ...
    protected void paintComponent(Graphics gr) {
        ...
        g.drawRect(
                (int) frameMargins,
                (int) (upperFrameHeight + frameMargins),
                (int) (width - frameMargins - 1),
                (int) lowerFrameHeight - 1);
        ...
        if (mBrd != null) {
            ...
            BezierBoardDrawUtil.paintBezierSplines(
                    jd,
                    outlineDrawPos,
                    upperFrameHeight - bottomMargin,
                    scale,
                    Color.BLACK,
                    stroke,
                    new BezierSpline[] { mBrd.getOutline() },
                    BezierBoardDrawUtil.Vertical | BezierBoardDrawUtil.FlipY | BezierBoardDrawUtil.MirrorX,
                    true);
            BezierBoardDrawUtil.paintBezierSplines(
                    jd,
                    outlineDrawPos + mBrd.getMaxWidth() * scale / 2 + mBrd.getThickness() * scale + 5.0 * scale,
                    upperFrameHeight - bottomMargin,
                    scale,
                    Color.BLACK,
                    stroke,
                    new BezierSpline[] { mBrd.getDeck(), mBrd.getBottom() },
                    BezierBoardDrawUtil.Vertical | BezierBoardDrawUtil.FlipX | BezierBoardDrawUtil.FlipY,
                    true);
            ...
            g.drawString(
                    ...
                    (int) ((upperFrameHeight + topMargin + fontMetrics.getHeight())));
            ...
            g.drawString(
                    modelStr,
                    (int) (width - strLength) / 2,
                    (int) ((upperFrameHeight + topMargin + fontMetrics.getHeight() * 2)));

            String dimStr = UnitUtils.convertLengthToCurrentUnit(mBrd.getLength(), true) + "("
                    + UnitUtils.convertLengthToCurrentUnit(mBrd.getLengthOverCurve(), true) + ") x "
                    + UnitUtils.convertLengthToCurrentUnit(mBrd.getMaxWidth(), false) + " x "
                    + UnitUtils.convertLengthToCurrentUnit(mBrd.getThickness(), false);
            strLength = fontMetrics.stringWidth(dimStr);
            g.drawString(
                    dimStr,
                    (int) (width - strLength) / 2,
                    (int) ((upperFrameHeight + topMargin + fontMetrics.getHeight() * 3)));

            String volumeStr = LanguageResource.getString("VOLUME_STR")
                    + UnitUtils.convertVolumeToCurrentUnit(mBrd.getVolume());
            strLength = fontMetrics.stringWidth(volumeStr);
            g.drawString(
                    volumeStr,
                    (int) (width - strLength) / 2,
                    (int) ((upperFrameHeight + topMargin + fontMetrics.getHeight() * 4)));

            String shaperAndSurferStr = ((!mBrd.getDesigner().isEmpty())
                    ? (LanguageResource.getString("DESIGNED_BY_STR") + mBrd.getDesigner() + " ")
                    : "") + ((!mBrd.getSurfer().isEmpty()) ? (LanguageResource.getString("FOR_STR") + mBrd.getSurfer()) : "");
            strLength = fontMetrics.stringWidth(shaperAndSurferStr);
            g.drawString(
                    shaperAndSurferStr,
                    (int) (width - strLength) / 2,
                    (int) ((upperFrameHeight + topMargin + fontMetrics.getHeight() * 5)));
        }
    }
