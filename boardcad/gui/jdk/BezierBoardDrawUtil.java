...
public class BezierBoardDrawUtil {
    ...
    public static GeneralPath makeBezierPathFromControlPoints(
            BezierSpline spline,
            boolean flipX,
            boolean flipY,
            boolean vertical,
        ...
        if (spline == null) return path;

        if (spline.getNrOfControlPoints() == 0) return path;
    ...
    public static void paintBezierSpline(
            AbstractDraw d,
            double offsetX,
            ...
            Color color,
            Stroke stroke,
            BezierSpline mPoints,
            int DrawControl,
            boolean fill) {
        if (mPoints == null) return;

        GeneralPath path = makeBezierPathFromControlPoints(
                mPoints,
                (DrawControl & BezierBoardDrawUtil.FlipX) != 0,
                (DrawControl & BezierBoardDrawUtil.FlipY) != 0,
                (DrawControl & BezierBoardDrawUtil.Vertical) != 0,
                false);
        paintPath(d, offsetX, offsetY, scale, color, stroke, path, fill);

        if ((DrawControl & BezierBoardDrawUtil.MirrorX) != 0) {
            path = makeBezierPathFromControlPoints(
                    mPoints,
                    !((DrawControl & BezierBoardDrawUtil.FlipX) != 0),
                    (DrawControl & BezierBoardDrawUtil.FlipY) != 0,
                    (DrawControl & BezierBoardDrawUtil.Vertical) != 0,
    ...
    public static void paintBezierSplines(
            AbstractDraw d,
            double offsetX,
        ...
        for (int i = 0; i < splines.length; i++) {
            GeneralPath tmp = makeBezierPathFromControlPoints(
                    splines[i],
                    (DrawControl & BezierBoardDrawUtil.FlipX) != 0,
                    (DrawControl & BezierBoardDrawUtil.FlipY) != 0,
                    (DrawControl & BezierBoardDrawUtil.Vertical) != 0,
    ...
    public static void paintBezierControlPoints(
            AbstractDraw d,
            double offsetX,
            ...
            Stroke stroke,
            BezierSpline mPoints,
            ArrayList<BezierKnot> mSelectedControlPoints,
            boolean flipX,
            boolean flipY) {
        if (mPoints == null) return;
        ...
        Color selTan1Color = (mSelectedControlPoints != null && mSelectedControlPoints.size() == 1)
                ? BoardCAD.getInstance().getSelectedTangent1ControlPointColor()
                : selControlPointColor;
        Color selTan2Color = (mSelectedControlPoints != null && mSelectedControlPoints.size() == 1)
                ? BoardCAD.getInstance().getSelectedTangent2ControlPointColor()
                : selControlPointColor;
        Color selOutControlPointColor = BoardCAD.getInstance().getSelectedOutlineCenterControlPointColor();
        Color selOutTan1Color = (mSelectedControlPoints != null && mSelectedControlPoints.size() == 1)
                ? BoardCAD.getInstance().getSelectedOutlineTangent1ControlPointColor()
                : selControlPointColor;
        Color selOutTan2Color = (mSelectedControlPoints != null && mSelectedControlPoints.size() == 1)
                ? BoardCAD.getInstance().getSelectedOutlineTangent2ControlPointColor()
                : selControlPointColor;
        ...
        for (int i = 0; i < mPoints.getNrOfControlPoints(); i++) {
            ...
            if (isControlPointSelected && mSelectedControlPoints.size() == 1 && BoardCAD.getInstance()
                    .getCurrentCommand()
                    .getClass()
                    .getName()
                    .compareTo(BrdEditCommand.class.getName()) == 0) {
            ...
            // ControlPoint
            d.setStroke(stroke);
            if (whichControlPointSelected == 0 || (mSelectedControlPoints != null && mSelectedControlPoints.size() > 1
                    && mSelectedControlPoints.contains(currentControlPoint))) {
                circle.setFrameFromCenter(
                        kx * mulX,
                        ky * mulY,
                        (kx * mulX) + (selectedCircleSize / scale),
                        (ky * mulY) + (selectedCircleSize / scale));
                ...
                circle.setFrameFromCenter(
                        kx * mulX,
                        ky * mulY,
                        (kx * mulX) + (unselectedCircleSize / scale),
                        (ky * mulY) + (unselectedCircleSize / scale));
            ...
            if (i != 0) {
                ...
                if (whichControlPointSelected == 1) {
                    circle.setFrameFromCenter(
                            cpx * mulX,
                            cpy * mulY,
                            (cpx * mulX) + (selectedCircleSize / scale),
                            (cpy * mulY) + (selectedCircleSize / scale));
                    ...
                    circle.setFrameFromCenter(
                            cpx * mulX,
                            cpy * mulY,
                            (cpx * mulX) + (unselectedCircleSize / scale),
                            (cpy * mulY) + (unselectedCircleSize / scale));
            ...
            if (i != mPoints.getNrOfControlPoints() - 1) {
                ...
                if (whichControlPointSelected == 2) {
                    circle.setFrameFromCenter(
                            cnx * mulX,
                            cny * mulY,
                            (cnx * mulX) + (selectedCircleSize / scale),
                            (cny * mulY) + (selectedCircleSize / scale));
                    ...
                    circle.setFrameFromCenter(
                            cnx * mulX,
                            cny * mulY,
                            (cnx * mulX) + (unselectedCircleSize / scale),
                            (cny * mulY) + (unselectedCircleSize / scale));
    ...
    public static void paintGuidePoints(
            AbstractDraw d,
            double offsetX,
            ...
            Color color,
            Stroke stroke,
            ArrayList<Point2D.Double> guidePoints,
            boolean flipX,
            boolean flipY) {
    ...
    public static void paintFins(
        ...
        // x, y for back of fin, x,y for front of fin, bac of center, front of center, depth of center, depth of sidefin, splay
        // angle
    ...
    public static void paintCircle(
        ...
        circle.setFrameFromCenter(
                pos.x * mulX,
                pos.y * mulY,
                (pos.x * mulX) + (circleSize / scale),
                (pos.y * mulY) + (circleSize / scale));
        d.draw(circle);
    ...
    public static void paintGrid(
            ...
            double scale,
            Color color,
            double verticalToCover,
            double horizontalToCover,
            boolean flipX,
            boolean flipY) {
    ...
    private static void paintGridMetric(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    private static void paintGridImperial(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void paintCurvature(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void paintVolumeDistribution(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void paintOutlineFlowLines(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void paintOutlineTuckUnderLine(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void paintProfileFlowLines(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void paintProfileApexline(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void paintProfileTuckUnderLine(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void paintCrossSectionCenterline(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void paintCrossSectionFlowLines(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void paintCrossSectionApexline(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void paintCrossSectionTuckUnderLine(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    static void paintCurvatureByT(
        ...
        if (Double.isInfinite(chord) || Double.isInfinite(length) || Double.isNaN(chord) || Double.isNaN(length)) return;

        if ((chord > CURVATURE_MAX_LINE_LENGTH || Math.abs(length - chord) > CURVATURE_TOLERANCE)
                && chord > CURVATURE_MIN_LINE_LENGTH && Math.abs(t1 - t0) > CURVATURE_T_TOLERANCE) {
    ...
    static void paintCurvatureByPos(
            AbstractDraw d,
            BezierCurve curve,
            double x0,
            double x1,
        ...
        if ((chord > CURVATURE_MAX_LINE_LENGTH || Math.abs(length - chord) > CURVATURE_TOLERANCE)
                && chord > CURVATURE_MIN_LINE_LENGTH) {
    ...
    public static void paintSlidingCrossSection(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double rotation,
            ...
            double pos,
            BezierBoard brd) {
        if (brd.getCrossSections().size() <= 2) return;
        ...
        case LinearInterpolation:
            paintSlidingCrossSectionBlendInterpolation(
                    d,
                    offsetX,
                    offsetY,
                    rotation,
        ...
        case ControlPointInterpolation:
            paintSlidingCrossSectionControlPointInterpolation(
                    d,
                    offsetX,
                    offsetY,
                    rotation,
        ...
        case SLinearInterpolation:
            // paintSlidingCrossSectionBlendFromSInterpolation(d, offsetX, offsetY, scale, color, stroke, flipX, flipY, pos,
            // brd);
            paintSlidingCrossSectionBlendFromSInterpolationSep(
                    d,
                    offsetX,
                    offsetY,
                    rotation,
    ...
    public static void paintSlidingCrossSectionNormals(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
        ...
        for (int w = 0; w < 3; w++) {
            ...
            for (int i = 1; i <= steps; i++) {
                point = AbstractBezierBoardSurfaceModel
                        .getBezierBoardSurfaceModel(BoardCAD.getInstance().getCrossSectionInterpolationType())
                        .getPointAt(mBrd, pos, (double) i / steps, a0, a1, true);
                if (point == null) continue;
                point.z -= rocker;

                normal = AbstractBezierBoardSurfaceModel
                        .getBezierBoardSurfaceModel(BoardCAD.getInstance().getCrossSectionInterpolationType())
                        .getNormalAt(mBrd, pos, (double) i / steps, a0, a1, true);
                ...
                line.setLine(
                        new Point2D.Double(point.y * mMulX, point.z * mMulY),
                        new Point2D.Double(otherEnd.y * mMulX, otherEnd.z * mMulY));
                d.draw(line);
            }
    ...
    /*
     * public static void paintSlidingCrossSectionBlendFromSInterpolation(AbstractDraw d, double offsetX, double offsetY, double
     * scale, Color color, Stroke stroke, boolean flipX, boolean flipY, double pos, BezierBoard brd) { // mMulX = flipX?-1:1;
     * mMulY = flipY?-1:1;
     * 
     * mBrd = brd;
     * 
    ...
    public static void paintSlidingCrossSectionBlendFromSInterpolationSep(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double rotation,
        ...
        double splitAngle = TUCK_UNDER_DEFINITION_ANGLE;
        /*
         * BezierBoardCrossSection c1 = brd.getPreviousCrossSection(pos); BezierBoardCrossSection c2 =
         * brd.getNextCrossSection(pos);
         * 
         * //Scaling double targetWidth = mBrd.getWidthAt(pos); double targetThickness = mBrd.getThicknessAtPos(pos);
         * 
         * double c1Width = c1.getWidth(); double c1Thickness = c1.getThicknessAtPos(BezierSpline.ZERO);
         * 
         * double c2Width = c2.getWidth(); double c2Thickness = c2.getThicknessAtPos(BezierSpline.ZERO);
         * 
         * double c1ThicknessScale = targetThickness/c1Thickness; double c1WidthScale = targetWidth/c1Width;
         * 
         * double c2ThicknessScale = targetThickness/c2Thickness; double c2WidthScale = targetWidth/c2Width;
         * 
        ...
        /*
         * double s1 = c1.getBezierSpline().getSByNormalReverseScaled(splitAngle, true, c1WidthScale, c1ThicknessScale); double
         * s2 = c2.getBezierSpline().getSByNormalReverseScaled(splitAngle, true, c2WidthScale, c2ThicknessScale);
         * 
         * // d.setColor(new Color(0,0,255)); paintSlidingCrossSectionBlendFromSInterpolation(pos, BezierSpline.ZERO, s1,
         * BezierSpline.ZERO, s2); // d.setColor(new Color(0,250,0)); paintSlidingCrossSectionBlendFromSInterpolation(pos, s1,
         * BezierSpline.ONE, s2, BezierSpline.ONE);
         */
        ...
        for (int w = 0; w < 3; w++) {
            ...
            GeneralPath path = new GeneralPath();
            Point3d point = AbstractBezierBoardSurfaceModel
                    .getBezierBoardSurfaceModel(AbstractBezierBoardSurfaceModel.ModelType.SLinearInterpolation)
                    .getPointAt(mBrd, pos, 0.0, a0, a1, true);
    ...
    /*
     * public static void paintSlidingCrossSectionBlendFromSInterpolation(double pos, double s0, double s1 ) { // Get endpoints
     * Point2D.Double p0 = mBrd.getPointAtPos(pos, s0); Point2D.Double p1 = mBrd.getPointAtPos(pos, s1);
     * 
     * // Get x split point double ss = (s1-s0)/2 + s0; Point2D.Double ps = mBrd.getPointAtPos(pos, ss);
     * 
     * // Angle between vectors formed by Point2D.Double v0 = new Point2D.Double(); Point2D.Double v1 = new Point2D.Double();
     * 
     ...
     * paintSlidingCrossSectionBlendFromSInterpolation(pos, s0, ss); paintSlidingCrossSectionBlendFromSInterpolation(pos, ss,
     * s1); } else { /*Draw Normal for debug Vector3d vec = mBrd.getNormalAtPos(pos, ss); ls.setLine(ps.x*mMulX,
     * ps.y*mMulY,(ps.x+vec.y)*mMulX, (ps.y+vec.z)*mMulY); ds.draw(ls);
     */
    /*
     * ls.setLine(p0.x*mMulX, p0.y*mMulY, ps.x*mMulX, ps.y*mMulY); ds.draw(ls); ls.setLine(ps.x*mMulX, ps.y*mMulY, p1.x*mMulX,
     * p1.y*mMulY); ds.draw(ls); ls.setLine(-p0.x*mMulX, p0.y*mMulY, -ps.x*mMulX, ps.y*mMulY); ds.draw(ls);
     * ls.setLine(-ps.x*mMulX, ps.y*mMulY, -p1.x*mMulX, p1.y*mMulY); ds.draw(ls); }
     * 
     * }
     */
    /*
     * public static void paintSlidingCrossSectionBlendFromSInterpolation(double pos, double a0, double a1, double b0, double b1
     * ) { // Get endpoints Point2D.Double p0 = mBrd.getPointAtPos(pos, a0, b0); Point2D.Double p1 = mBrd.getPointAtPos(pos, a1,
     * b1);
     * 
     * // Get x split point double as = (a1-a0)/2 + a0; double bs = (b1-b0)/2 + b0; Point2D.Double ps = mBrd.getPointAtPos(pos,
     * as, bs);
     * 
     * // Angle between vectors formed by Point2D.Double v0 = new Point2D.Double(); Point2D.Double v1 = new Point2D.Double();
     * 
     ...
     * paintSlidingCrossSectionBlendFromSInterpolation(pos, as, a1, bs, b1); } else { /* Draw normal for debug Vector3d vec =
     * mBrd.getNormalAtPos(pos, as, bs); ls.setLine(ps.x*mMulX, ps.y*mMulY,(ps.x+vec.y)*mMulX, (ps.y+vec.z)*mMulY); ds.draw(ls);
     * 
     * Draw tangent for debug vec = mBrd.getTangentAtPos(pos, as, bs); ls.setLine(ps.x*mMulX, ps.y*mMulY,(ps.x+vec.y)*mMulX,
     * (ps.y+vec.z)*mMulY); ds.setColor(new Color(0,0,250)); ds.draw(ls); ds.setColor(new Color(255,255,0));
     */
    /*
     * ls.setLine(p0.x*mMulX, p0.y*mMulY, ps.x*mMulX, ps.y*mMulY); ds.draw(ls); ls.setLine(ps.x*mMulX, ps.y*mMulY, p1.x*mMulX,
     * p1.y*mMulY); ds.draw(ls); ls.setLine(-p0.x*mMulX, p0.y*mMulY, -ps.x*mMulX, ps.y*mMulY); ds.draw(ls);
     ...
     * }
     */
    public static void paintSlidingCrossSectionControlPointInterpolation(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double rotation,
        ...
        if (interpol != null) {
            BezierBoardDrawUtil.paintBezierSpline(
                    ...
                    color,
                    stroke,
                    interpol.getBezierSpline(),
                    ((flipX == true) ? BezierBoardDrawUtil.FlipX : 0) | ((flipY == true) ? BezierBoardDrawUtil.FlipY : 0),
                    true);
            BezierBoardDrawUtil.paintBezierSpline(
                    d,
                    offsetX,
                    ...
                    stroke,
                    interpol.getBezierSpline(),
                    ((flipX == true) ? 0 : BezierBoardDrawUtil.FlipX) | ((flipY == true) ? BezierBoardDrawUtil.FlipY : 0),
                    true);
            // DEBUG BrdDrawUtil.paintBezierControlPoints(d,offsetX, offsetY, scale, stroke,
            // interpol.mCrossSectionControlPoints, new ArrayList<ControlPoint>(), flipX, flipY); //DEBUG!
        }

    }

    public static void paintSlidingCrossSectionBlendInterpolation(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double rotation,
     ...
     * 
     * // Get endpoints double z0 = mBrd.getDeckAtPos(x, y0); double z1 = mBrd.getDeckAtPos(x, y1);
     * 
     * // Get x split point double ys = (y1-y0)/2 + y0; double zs = mBrd.getDeckAtPos(x, ys);
     * 
     * // Distance between centerpoint and real split curvepoint double length = BezierPatch.getVecLength(y0,z0,ys,zs) +
     * BezierPatch.getVecLength(ys,zs,y1,z1); double chord = BezierPatch.getVecLength(y0,z0,y1,z1);
     * 
     * if(Double.isInfinite(chord) || Double.isInfinite(length) || Double.isNaN(chord) || Double.isNaN(length) ) return;
     * 
     * if(Math.abs(length - chord) > SLIDING_CROSS_SECTION_TOLERANCE && chord > SLIDING_CROSS_SECTION_MIN_LINE_LENGTH ) {
     * paintSlidingCrossSection(x, y0, ys); paintSlidingCrossSection(x, ys, y1); } else { ls.setLine(y0*mMulX, z0*mMulY,
     * ys*mMulX, zs*mMulY); ds.draw(ls); ls.setLine(ys*mMulX, zs*mMulY, y1*mMulX, z1*mMulY); ds.draw(ls); }
     * 
     * }
     */
    public static void paintSlidingCrossSectionBlendInterpolation(AbstractDraw d, double x, double y0, double y1) {
        ...
        if (Math.abs(y1 - y0) > SLIDING_CROSS_MIN_SPLIT
                && (chord > SLIDING_CROSS_SECTION_MAX_LINE_LENGTH || (Math.abs(length - chord) > SLIDING_CROSS_SECTION_TOLERANCE
                        && chord > SLIDING_CROSS_SECTION_MIN_LINE_LENGTH))) {
    ...
    public static void paintDetails(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
            ...
            double posOffsetX,
            double posOffsetY,
            double measurementOffsetX,
            double measurementOffsetY) {
        for (int i = 0; i < beziers.length; i++) {
            paintBezierSpline(
                    ...
                    beziers[i],
                    ((flipX == true) ? BezierBoardDrawUtil.FlipX : 0) | ((flipY == true) ? BezierBoardDrawUtil.FlipY : 0)
                            | ((mirrorX == true) ? BezierBoardDrawUtil.MirrorX : 0)
                            | ((mirrorY == true) ? BezierBoardDrawUtil.MirrorY : 0),
                    true);
        }
    }

    public static void paintDetailsVertical(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static AffineTransform setTransform(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void printOutline(
        ...
        // paintBezierControlPoints(d,offsetX, offsetY, scale, BoardCAD.getInstance().getBrdColor(), stroke,
        ...
        if (printGuidePoints == true) {
            BezierBoardDrawUtil.paintGuidePoints(
                    d,
                    offsetX,
                    offsetY,
                    scale,
    ...
    public static void printOutlineOverCurve(
            AbstractDraw d,
            double offsetX,
        ...
        // paintBezierControlPoints(d,offsetX, offsetY, scale, BoardCAD.getInstance().getBrdColor(), stroke,
        // brd.mDeckControlPoints, false, false);
    ...
    public static void printSpinTemplate(
            AbstractDraw d,
            double offsetX,
        ...
        paintPath(
                ...
                scale,
                BoardCAD.getInstance().getBrdColor(),
                stroke,
                outlineUpper,
                false);
    ...
    public static void printSpinTemplateOverCurve(
            AbstractDraw d,
            double offsetX,
    ...
    public static void printProfile(
        ...
        if (printGuidePoints == true) {
            BezierBoardDrawUtil
                    .paintGuidePoints(d, offsetX, offsetY, scale, Color.GRAY, stroke, brd.getDeckGuidePoints(), false, false);
            BezierBoardDrawUtil
                    .paintGuidePoints(d, offsetX, offsetY, scale, Color.GRAY, stroke, brd.getBottomGuidePoints(), false, false);
        }
    }
    ...
    public static void printSlice(
        ...
        if (printGuidePoints == true) {
            BezierBoardDrawUtil
                    .paintGuidePoints(d, offsetX, offsetY, scale, Color.GRAY, stroke, crs.getGuidePoints(), false, false);
        }
    }

    // Old functions orginating from printing
    public static void printRailTemplate(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
        ...
        while (brd.getLength() / 2.54 > verticalLines)
            verticalLines += 6;
        while ((brd.getMaxRocker() / 2) / 2.54 > horizontalLines)
            horizontalLines += 6;
        ...
        for (; i <= steps; i++) {
            ...
            if (first && deck < bottom) continue;

            if (!first && deck < bottom) break;
    ...
    // Old functions orginating from printing
    public static void printDeckSkinTemplate(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
        ...
        while (brd.getLength() / 2.54 > verticalLines)
            verticalLines += 6;
        while ((brd.getMaxWidth() / 2) / 2.54 > horizontalLines)
            horizontalLines += 6;
    ...
    // Old functions orginating from printing
    public static void printBottomSkinTemplate(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void printCrossSection(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void paintFunction(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
        ...
        for (int i = 1; i < splits; i++) {
            ...
            l.setLine(
                    x * horizontalScale,
                    lastValue * verticalScale,
                    (x + step) * horizontalScale,
                    currentValue * verticalScale);
            d.draw(l);
