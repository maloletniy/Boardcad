...
public class PrintHollowWoodTemplates extends JComponent implements Printable {
    ...
    public static void printStringerWebbing(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void printCrossSectionWebbing(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void printRailWebbing(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void printRailNotching(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void printTailPiece(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void printTailPieceWebbing(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void printStringerTailPieceCutOut(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void printRailTailPieceNotches(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
        ...
        for (int n = 0; n < 5; n++) {
            ...
            for (int i = 0; i < steps; i++) {
                ...
                if (deck < bottom) break;

                if (first) {
                    // Calculate angle for tailpiece (tail piece sit halfway between deck and bottom on the rail template at
                    // both ends)
            ...
            /*
             * Line2D.Double line = new Line2D.Double(); line.setLine(lastPos, deck, lastPos, deck + 8.0); d.draw(line);
             */
    ...
    public static void printNosePiece(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void printNosePieceWebbing(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
        ...
        for (int n = 0; n < 2; n++) {
            ...
            for (int i = 0; i < steps; i++) {
                ...
                if (first) {
                    ...
                    // Calculate angle for nosepiece (Nose piece sit halfway between deck and bottom on the rail template at
                    // both ends)
                    double z1 = (bottom + deck) / 2.0;
    ...
    public static void printStringerNosePieceCutOut(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
    ...
    public static void printRailNosePieceNotches(
            AbstractDraw d,
            double offsetX,
            double offsetY,
            double scale,
        ...
        for (int n = 0; n < 6; n++) {
            ...
            for (int i = 0; i < steps; i++) {
                ...
                if (!endFound && n > 0) {
                    ...
                    first = false;
                    /*
                     * Line2D.Double point = new Line2D.Double(); point.setLine(lastPos, endPointZ,lastPos, endPointZ);
                     * d.draw(point);
                     */
                    thicknessXOffset = Math.sin(angle) * frameThickness / 2.0;
                    thicknessYOffset = Math.cos(angle) * frameThickness / 2.0;
                ...
                if (!first) {
                    ...
                    double newPos = lastPos + stepLength;
                    /*
                     * Line2D.Double line = new Line2D.Double(); line.setLine(lastPos, bottom, newPos, bottom); d.draw(line);
                     */
                    // Update last pos
                    lastPos = newPos;
                } else {
            ...
            if (n % 2 == 1) {
                ...
                /*
                 * Line2D.Double line = new Line2D.Double(); line.setLine(notchX, notchZ, notchX, notchZ); d.draw(line);
                 */ } else if (n % 2 == 0 && n > 0) {
                double otherEndX = lastPos;
                double otherEndZ = endPointZ + (((ox - endPointX) / Math.cos(angle)) * Math.sin(angle));
                /* System.out.printf("otherEndX: %f, otherEndZ: %f\n", otherEndX, otherEndZ); */
                /*
                 * Line2D.Double line = new Line2D.Double(); line.setLine(otherEndX, otherEndZ, otherEndX, otherEndZ);
                 * d.draw(line); path.moveTo(notchX, notchZ); path.lineTo(otherEndX, otherEndZ);
                 */
                d.setColor(new Color(255, 0, 0));
        ...
        /*
         * Line2D.Double line = new Line2D.Double(); d.setColor(new Color(0,255,0)); line.setLine(endPointPos, endPointZ,
         * lastPos, deck+bottom/2); d.draw(line);
         *
         * d.setColor(new Color(0,0,255)); line.setLine(endPointPos, endPointZ, lastPos, endPointZ +
         * (((ox-step-endPointX)/Math.cos(angle))*Math.sin(angle))); d.draw(line);
         */
        d.setTransform(savedTransform);
    }

    // helper functions
    static double getTailPieceStringerAngle(
            BezierBoard brd,
            double distanceFromRail,
            double skinThickness,
            double frameThickness,
    ...
    static double getTailPieceRailAngle(
            BezierBoard brd,
            double distanceFromRail,
            double skinThickness,
            double frameThickness,
    ...
    static double getNosePieceStringerAngle(
            BezierBoard brd,
            double distanceFromRail,
            double skinThickness,
            double frameThickness,
    ...
    static double getNosePieceRailAngle(
            BezierBoard brd,
            double distanceFromRail,
            double skinThickness,
            double frameThickness,
    ...
    public void printStringerTemplate(
            double skinThickness,
            double frameThickness,
            double webbing,
            double tailOffset,
    ...
    public void printCrosssectionTemplates(
            double distanceFromRail,
            double skinThickness,
            double frameThickness,
            double webbing) {
        ...
        try {
            ...
            int nrOfCrossSections = (int) ((BoardCAD.getInstance().getCurrentBrd().getLength() - 9.0 * UnitUtils.INCH)
                    / UnitUtils.FOOT);
    ...
    public void paintComponent(Graphics g) {
        ...
        // BezierBoardDrawUtil.printProfile(jd, border, dim.height*2.0/5.0,
        // (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()),
        ...
        /*
         * BezierBoardDrawUtil.printProfile(jd, border, dim.height*2.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), true,
         * BoardCAD.getInstance().getCurrentBrd(), 0.0, mSkinThickness, false, 0.0, 0.0);
         *
         * BezierBoardDrawUtil.printProfile(jd, border, dim.height*2.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), true,
         * BoardCAD.getInstance().getCurrentBrd(), 0.0, 0.0, false, 0.0, 0.0);
         *
         * printStringerWebbing(jd, border, dim.height*1.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), BoardCAD.getInstance().getCurrentBrd(),
        ...
        /*
         * printCrossSection(jd, border, dim.height*2.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), BoardCAD.getInstance().getCurrentBrd(),
         * 3*UnitUtils.FOOT, 2*UnitUtils.INCH, mSkinThickness, mFrameThickness, mWebbing, false);
         *
         * printCrossSection(jd, border, dim.height*2.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), BoardCAD.getInstance().getCurrentBrd(),
         * 3*UnitUtils.FOOT, 2*UnitUtils.INCH, mSkinThickness, mFrameThickness, mWebbing, true);
         *
         * printCrossSectionWebbing(jd, border, dim.height*2.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), BoardCAD.getInstance().getCurrentBrd(),
         * 3*UnitUtils.FOOT, 2*UnitUtils.INCH, mSkinThickness, mFrameThickness, mWebbing, false);
         *
         * printCrossSectionWebbing(jd, border, dim.height*2.0/5.0,
         ...
         *
         *
         * BezierBoardDrawUtil.printRailTemplate(jd, border, dim.height*3.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), true,
         * BoardCAD.getInstance().getCurrentBrd(), mDistanceToRail, mSkinThickness, mTailOffset, mNoseOffset, false);
         *
         * printRailWebbing(jd, border, dim.height*3.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), BoardCAD.getInstance().getCurrentBrd(),
         * mDistanceToRail, mSkinThickness, mFrameThickness, mWebbing, mTailOffset, mNoseOffset);
         *
         * printRailNotching(jd, border, dim.height*3.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), BoardCAD.getInstance().getCurrentBrd(),
         * mDistanceToRail, mSkinThickness, mFrameThickness, mWebbing, mTailOffset, mNoseOffset);
         *
         * printRailTailPieceNotches(jd, border, dim.height*3.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), BoardCAD.getInstance().getCurrentBrd(),
         * mDistanceToRail, mSkinThickness, mFrameThickness, mWebbing, mTailOffset, mNoseOffset);
         *
         * printRailNosePieceNotches(jd, border, dim.height*3.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), BoardCAD.getInstance().getCurrentBrd(),
         * mDistanceToRail, mSkinThickness, mFrameThickness, mWebbing, mTailOffset, mNoseOffset);
         *
         *
         * printTailPiece(jd, border, dim.height*3.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), BoardCAD.getInstance().getCurrentBrd(),
         * mDistanceToRail, mSkinThickness, mFrameThickness, mWebbing, mTailOffset, false);
         *
         * printTailPiece(jd, border, dim.height*3.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), BoardCAD.getInstance().getCurrentBrd(),
         * mDistanceToRail, mSkinThickness, mFrameThickness, mWebbing, mTailOffset, true);
         *
         * printTailPieceWebbing(jd, border, dim.height*3.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), BoardCAD.getInstance().getCurrentBrd(),
         * mDistanceToRail, mSkinThickness, mFrameThickness, mWebbing, mTailOffset, false);
         *
         * printTailPieceWebbing(jd, border, dim.height*3.0/5.0,
        ...
        printNosePiece(
                ...
                false);
        /*
         * printNosePiece(jd, border, dim.height*3.0/5.0,
         * 3*(dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()),
         * BoardCAD.getInstance().getCurrentBrd(), mDistanceToRail, mSkinThickness, mFrameThickness, mWebbing, mNoseOffset,
         * true);
         *
         * printNosePieceWebbing(jd, border, dim.height*3.0/5.0,
         * 3*(dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()),
         * BoardCAD.getInstance().getCurrentBrd(), mDistanceToRail, mSkinThickness, mFrameThickness, mWebbing, mNoseOffset,
         * false);
         *
    ...
    public int print(Graphics g, PageFormat pageFormat, int pageIndex) {
        ...
        case PRINT_STRINGER_TEMPLATE: {
            if (printStringerTemplate(pageFormat, pageIndex, g) == 0) return PAGE_EXISTS;
    ...
    int printStringerTemplate(PageFormat pageFormat, int pageIndex, Graphics g) {
        System.out.printf(
                "printStringerTemplate() Page width: %f, page height: %f, orientation: %s, Margins x:%f, y: %f\n",
        ...
        int widthInPages = (int) (BoardCAD.getInstance().getCurrentBrd().getLength() / ((width / 72) * 2.54)) + 2;

        int heightInPages = (int) (BoardCAD.getInstance().getCurrentBrd().getMaxRocker() / ((height / 72) * 2.54)) + 1;
        ...
        if (pageIndex < widthInPages * heightInPages) {
            ...
            String mModelString = LanguageResource.getString("BOARDFILE_STR")
                    + BoardCAD.getInstance().getCurrentBrd().getFilename() + LanguageResource.getString("STRINGER_STR");
