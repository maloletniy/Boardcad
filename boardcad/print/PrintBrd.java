...
public class PrintBrd extends JComponent implements Printable {
    ...
    static PageFormat getPageFormat(Printable printable, PrinterJob printJob, double targetWidth) {
        if (!printJob.printDialog()) return null;

        PageFormat retPage;
        PageFormat page = printJob.defaultPage();
        Paper paper = page.getPaper();
        paper.setImageableArea(
                BoardCAD.mPrintMarginLeft,
                BoardCAD.mPrintMarginTop,
                paper.getWidth() - (BoardCAD.mPrintMarginLeft + BoardCAD.mPrintMarginRight),
                paper.getHeight() - (BoardCAD.mPrintMarginTop + BoardCAD.mPrintMarginBottom));
        page.setPaper(paper);

        retPage = printJob.pageDialog(page);
        if (retPage == page) return null;
        ...
        System.out.printf(
                "Pre validation Page width: %f, page height: %f, orientation: %s, Margins x:%f, y:%f\n",
                retPage.getImageableWidth(),
                retPage.getImageableHeight(),
                retPage.getOrientation() == PageFormat.LANDSCAPE ? "Landscape" : "Portrait",
                retPage.getImageableX(),
                retPage.getImageableY());
    ...
    public void paintComponent(Graphics g) {
        ...
        // BezierBoardDrawUtil.printOutline(jd, border, dim.height*1.0/5.0, 0.0,
        // (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), true,
        // BoardCAD.getInstance().getCurrentBrd(), false, false);
        // BezierBoardDrawUtil.printOutlineOverCurve(jd, border, dim.height*1.0/5.0, 0.0,
        // (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), true,
        // BoardCAD.getInstance().getCurrentBrd(), false, false, false, false);

        // BezierBoardDrawUtil.printSpinTemplate(jd, border, dim.height*2.0/5.0, 0.0,
        // (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), true,
        // BoardCAD.getInstance().getCurrentBrd(), false, false);
        // BezierBoardDrawUtil.printSpinTemplateOverCurve(jd, border, dim.height*2.0/5.0, 0.0,
        // (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()), true,
        // BoardCAD.getInstance().getCurrentBrd(), false, false);
        /*
         * BezierBoardDrawUtil.printProfile(jd, border, dim.height*3.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()),
         * true,BoardCAD.getInstance().getCurrentBrd(),false); BezierBoardDrawUtil.printRailTemplate(jd, border,
         * dim.height*3.0/5.0, (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()),
         * true,BoardCAD.getInstance().getCurrentBrd(), 3.0);
         *
         * for(int i = 1; i < BoardCAD.getInstance().getCurrentBrd().getCrossSections().size()-1; i++) {
         * BezierBoardDrawUtil.printSlice(jd, border, dim.height*4.0/5.0,
         * (dim.width-(border*2))/(BoardCAD.getInstance().getCurrentBrd().getLength()),
         * true,BoardCAD.getInstance().getCurrentBrd(), i,false); }
         *
         * final BezierSpline p =
         * BoardCAD.getInstance().getCurrentBrd().getInterpolatedCrossSection(BoardCAD.getInstance().getCurrentBrd().getLength()
         * /2.0).getBezierSpline();
         *
         ...
         *
         * BezierBoardDrawUtil.paintFunction(jd, border, dim.height/2.0, scale, new Color(0,0,0), new
         * BasicStroke((float)(2.0/scale)), func, 0.0, 1.0, 200.0, 10.0);
         */
    }
    ...
    public int print(Graphics g, PageFormat pageFormat, int pageIndex) {
        ...
        case PRINT_OUTLINE: {
            if (printOutline(pageFormat, pageIndex, g) == 0) return PAGE_EXISTS;

            break;
        }
        case PRINT_SPINTEMPLATE: {
            if (printSpinTemplate(pageFormat, pageIndex, g) == 0) return PAGE_EXISTS;

            break;
        }

        case PRINT_PROFILE: {
            if (printProfile(pageFormat, pageIndex, g) == 0) return PAGE_EXISTS;

            break;
        }

        case PRINT_SLICES: {
            if (printSlices(pageFormat, pageIndex, g) == 0) return PAGE_EXISTS;
    ...
    int printOutline(PageFormat pageFormat, int pageIndex, Graphics g) {
        ...
        if (pageIndex < widthInPages * heightInPages) {
            ...
            String mModelString = LanguageResource.getString("BOARDFILE_STR")
                    + BoardCAD.getInstance().getCurrentBrd().getFilename() + LanguageResource.getString("OUTLINE_STR");
            ...
            if (mOverCurve) {
                BezierBoardDrawUtil.printOutlineOverCurve(
                        new JavaDraw(g2d),
                        ...
                        BoardCAD.getInstance().isPrintingControlPoints(),
                        BoardCAD.getInstance().isPrintingFins(),
                        false,
                        false);
            } else {
                BezierBoardDrawUtil.printOutline(
                        new JavaDraw(g2d),
    ...
    int printSpinTemplate(PageFormat pageFormat, int pageIndex, Graphics g) {
        ...
        if (pageIndex < widthInPages * heightInPages) {
            ...
            String mModelString = LanguageResource.getString("BOARDFILE_STR") + brd.getFilename()
                    + LanguageResource.getString("OUTLINE_STR");
            String mRowString = LanguageResource.getString("ROW_STR") + ((pageIndex % widthInPages) + 1) + "/" + widthInPages;
            String mColumnString = LanguageResource.getString("COLUMN_STR") + ((pageIndex / widthInPages) + 1) + "/"
                    + heightInPages;
            ...
            if (mOverCurve) {
                BezierBoardDrawUtil.printSpinTemplateOverCurve(
                        new JavaDraw(g2d),
                        -pageFormat.getImageableWidth() * (pageIndex % widthInPages),
                        -pageFormat.getImageableHeight() * (pageIndex / widthInPages),
                        0.0,
                ...
                BezierBoardDrawUtil.printSpinTemplate(
                        new JavaDraw(g2d),
                        -pageFormat.getImageableWidth() * (pageIndex % widthInPages),
                        -pageFormat.getImageableHeight() * (pageIndex / widthInPages),
                        0.0,
    ...
    int printProfile(PageFormat pageFormat, int pageIndex, Graphics g) {
        ...
        if (pageIndex < widthInPages * heightInPages) {
            ...
            String mRowString = LanguageResource.getString("ROW_STR") + ((pageIndex % widthInPages) + 1) + "/" + widthInPages;
            String mColumnString = LanguageResource.getString("BOARDFILE_STR") + ((pageIndex / widthInPages) + 1) + "/"
                    + heightInPages;
    ...
    int printSlices(PageFormat pageFormat, int pageIndex, Graphics g) {
        ...
        if (pageIndex < totalWidthInPages * heightInPages) {
            ...
            String mSliceString = LanguageResource.getString("CROSSECTION_STR") + (pageIndex / widthPrSliceInPages + 1)
                    + LanguageResource.getString("AT_STR")
                    + UnitUtils
                            .convertLengthToCurrentUnit(
                                    BoardCAD.getInstance()
                                            .getCurrentBrd()
                                            .getCrossSections()
                                            .get((pageIndex / widthPrSliceInPages + 1))
                                            .getPosition(),
                                    false);
            String mRowString = LanguageResource.getString("ROW_STR") + ((pageIndex % widthPrSliceInPages) + 1) + "/"
                    + widthPrSliceInPages;
            ...
            BezierBoardDrawUtil.printSlice(
                    new JavaDraw(g2d),
                    ...
                    true,
                    BoardCAD.getInstance().getCurrentBrd(),
                    pageIndex / widthPrSliceInPages + 1,
                    BoardCAD.getInstance().isPrintingControlPoints());
    ...
    int printSpecSheet(PageFormat pageFormat, int pageIndex, Graphics g) {
        ...
        String measurementsString = LanguageResource.getString("MEASUREMENTS_STR")
                + UnitUtils.convertLengthToCurrentUnit(brd.getLength(), true) + LanguageResource.getString("BY_STR")
                + UnitUtils.convertLengthToCurrentUnit(brd.getCenterWidth(), true) + LanguageResource.getString("BY_STR")
                + UnitUtils.convertLengthToCurrentUnit(brd.getThickness(), true);
        ...
        // Fins. x, y for back of fin, x,y for front of fin, back of center, front of center, depth of center, depth of sidefin,
        // splay angle
        /*
         * g2d.setColor(Color.LIGHT_GRAY); final Line2D tmp = new Line2D.Double();
         * tmp.setLine(brd.getFins[0],brd.getFins[1],brd.getFins[2],brd.getFins[3]); g2d.draw(tmp);
         * tmp.setLine(brd.getFins[0],-brd.getFins[1],brd.getFins[2],-brd.getFins[3]); g2d.draw(tmp);
         * tmp.setLine(brd.getFins[5],0,brd.getFins[4],0); g2d.draw(tmp);
         */
        ...
        BezierBoardDrawUtil.paintSlidingCrossSection(
                ...
                false,
                true,
                UnitUtils.INCHES_PR_FOOT * UnitUtils.INCH,
                brd);
        ...
        BezierBoardDrawUtil.paintSlidingCrossSection(
                ...
                false,
                true,
                brd.getLength() - UnitUtils.INCHES_PR_FOOT * UnitUtils.INCH,
                brd);
        ...
        if (brd.getFins()[2] != 0 || brd.getFins()[5] != 0 || brd.getFinType() != "") {
            ...
            if (brd.getFins()[5] != 0) {
                g.drawString(
                        LanguageResource.getString("CENTERFIN_STR") + LanguageResource.getString("FRONT_STR")
                                + UnitUtils.convertLengthToCurrentUnit(brd.getFins()[5], false)
                                + LanguageResource.getString("REAR_STR")
                                + UnitUtils.convertLengthToCurrentUnit(brd.getFins()[4], false)
                                + LanguageResource.getString("DEPTH_STR")
                                + UnitUtils.convertLengthToCurrentUnit(brd.getFins()[6], false),
                        xm,
                        currentY);
                currentY += hgt + 5;
            }
            ...
            if (brd.getFins()[2] != 0) {
                g.drawString(
                        LanguageResource.getString("SIDEFINS_STR") + LanguageResource.getString("FRONT_STR")
                                + UnitUtils.convertLengthToCurrentUnit(brd.getFins()[2], false)
                                + LanguageResource.getString("REAR_STR")
                                + UnitUtils.convertLengthToCurrentUnit(brd.getFins()[0], false)
                                + LanguageResource.getString("DEPTH_STR")
                                + UnitUtils.convertLengthToCurrentUnit(brd.getFins()[7], false)
                                + LanguageResource.getString("TOEIN_STR")
                                + UnitUtils.convertLengthToCurrentUnit(brd.getFins()[3] - brd.getFins()[1], false)
                                + LanguageResource.getString("SPLAY_STR")
                                + UnitUtils.convertLengthToCurrentUnit(brd.getFins()[8], false),
                        xm,
                        currentY);
                currentY += hgt + 5;
            }
